...
void parseAssemblyFile(workerData.bytes, workerData.filename, workerData.unit, workerData.upAxis).then(result => parentPort?.postMessage(result)).catch(error => {
  ...
  const message = error instanceof Error && /^(Use |Model |Invalid |Unsupported |Truncated |Multiple |Export |Missing |GLB |STL |STEP |PCB )/u.test(error.message) ? error.message : "Model could not be read. Check the file or export a simpler assembly.";
