...
async function setup(runtime: ProductionRuntime, itemId = "reconciliation-item") {
  ...
  const line = await service.createBomLine(revision.data.id, { id: `${itemId}-line`, name: "Reconciliation part", role: "consumed", itemId, requiredQuantity: 2, unit: "each", optional: false, alternatives: [], constraints: {} }, context());
