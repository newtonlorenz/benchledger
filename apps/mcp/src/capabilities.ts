...
export const TOOL_DEFINITIONS: readonly McpToolDefinition[] = [
  ...
  tool("update_bom_line", "Update one BOM requirement using optimistic versioning and evidence-bearing alternatives. Role consumed or reusable controls stock behavior; null clears the role to legacy review-required state. Only consumed requirements may reserve, consume, or reconcile.", "bom:write", true, { bomLineId: idProperty("BOM line identifier."), expectedVersion: integer(), description: string(), quantity: number(), unit: string(), requirement: string(), role: bomRequirementRoleProperty, itemId: idProperty("Exact inventory item, when known."), alternatives: array(bomAlternativeProperty), compatibleItemIds: array(string("Deprecated IDs-only alternative list; use alternatives for compatibility evidence.")), constraints: bomConstraintsProperty, notes: string() }, ["bomLineId"]),
