...
export const TOOL_DEFINITIONS: readonly McpToolDefinition[] = [
  ...
  tool("read_project_revision", "Read one project planning revision, its status, and persisted summary.", "projects:read", false, { revisionId: idProperty("Project revision identifier.") }, ["revisionId"]),
  ...
  tool("save_reconciliation_draft", "Save an evidence-bearing post-project close-out draft and server-calculated preview. First list reservations and submit/account only lines with active reservations; zero-reservation lines may be omitted, while one explicit reviewed_no_change outcome remains supported as an optional sole outcome. The preview and staleness basis still retain every BOM line, reservation, and source item. Draft saves never change stock or reservations and support optimistic version checks. This save is a separate command from commit and must not share its idempotency key.", "bom:write", true, { projectRevisionId: idProperty("Project revision identifier."), draftId: idProperty("Existing draft identifier."), expectedVersion: integer("Version read before editing."), lines: array(reconciliationLineProperty) }, ["projectRevisionId", "lines"]),
  tool("commit_reconciliation", "Explicitly commit a reviewed close-out. Every active reservation must be fully and exactly accounted for; zero-reservation BOM lines may be omitted, and reviewed_no_change is an optional explicit sole outcome for such a line. The server applies stock, reservation, and reusable-asset changes atomically using the full BOM/reservation/item basis; an ambiguous retry with the commit command's original idempotency key and identical payload replays safely.", "bom:write", true, { projectRevisionId: idProperty("Project revision identifier."), draftId: idProperty("Draft to commit."), expectedVersion: integer("Draft version read before confirmation.") }, ["projectRevisionId", "draftId"]),
