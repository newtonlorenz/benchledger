...
export function WorkstreamPlanning({ project, readOnly = false, onProjectRefresh }: { project: Project; readOnly?: boolean; onProjectRefresh?: (() => Promise<boolean>) | undefined }) {
  ...
  return <section className="surface workstream-planning" aria-label="Workstreams">
    ...
    <div className="workflow-pagination">{cursor && <button type="button" className="button button-quiet" onClick={() => navigation ? navigation.request(() => setCursor(undefined)) : setCursor(undefined)}>First workstreams</button>}{source.data?.nextCursor && <button type="button" className="button button-quiet" onClick={() => navigation ? navigation.request(() => setCursor(source.data!.nextCursor)) : setCursor(source.data!.nextCursor)}>Next workstreams</button>}<button type="button" className="text-button" disabled={creating || source.loading} onClick={() => navigation ? navigation.request(source.reload) : source.reload()}>Refresh workstreams</button></div>
...
function NewWorkstream({ root, onCancel, onSaved }: { root: string; onCancel(): void; onSaved(): void }) {
  ...
  useUnsavedWork(Boolean(name.trim()) || kind !== "assembly", "new workstream", command.busy || command.uncertain);
