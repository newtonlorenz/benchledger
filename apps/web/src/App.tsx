...
function App() {
  ...
  return (
    <div className="app-shell">
      ...
      {selectedItem && <InventoryDrawer item={selectedItem} categories={categories} categoriesLoading={categoriesLoading} categoriesError={categoriesError} expert={expert} onClose={() => setSelectedItemId(undefined)} onCount={recordCount} onCommission={commissionInventoryItem} onUpdate={updateInventoryItem} onCreateReplacement={(record) => { setSelectedItemId(undefined); setReplacementFor(record); setShowNewItem(true); }} />}
...
/** A stable, human-readable discriminator for duplicate physical records. */
export function inventoryDiscriminator(item: InventoryItem, expert = false): string {
  ...
  const source = item.provenance?.source?.trim();
  const location = item.location?.trim() && item.location.trim().toLocaleLowerCase() !== "unassigned" ? item.location.trim() : undefined;
  const generic = new Set([item.kind, item.category].filter(Boolean).map((value) => String(value).toLocaleLowerCase()));
  const variant = item.variant?.trim() && !generic.has(item.variant.trim().toLocaleLowerCase()) ? item.variant.trim() : undefined;
  const parts = [colour, catalogVariant, lot, asset, location, item.sku?.trim(), item.catalogProduct?.productCode, variant, source && expert ? source : undefined]
    .filter((part): part is string => Boolean(part));
  return [...new Set(parts)].slice(0, 2).join(" · ") || (expert ? item.id : "Physical item");
}

/** Keep unique choices clean while making same-named physical records distinct. */
export function inventoryCandidateLabel(item: InventoryItem, items: readonly InventoryItem[], expert = false): { name: string; discriminator?: string } {
  const duplicates = items
    .filter((candidate) => candidate.name.trim().toLocaleLowerCase() === item.name.trim().toLocaleLowerCase())
    .sort((left, right) => left.id.localeCompare(right.id));
  if (duplicates.length < 2) return { name: item.name };
  const evidence = inventoryDiscriminator(item, expert);
  if (evidence !== "Physical item") return { name: item.name, discriminator: evidence };
  const ordinal = Math.max(duplicates.findIndex((candidate) => candidate.id === item.id), 0) + 1;
  return { name: item.name, discriminator: `Physical item ${ordinal} of ${duplicates.length}` };
}

function inventoryCandidateText(item: InventoryItem, items: readonly InventoryItem[], expert = false): string {
