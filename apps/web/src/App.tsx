...
function BrandMark() { return ( <div className="brand-mark" aria-hidden="true"> <span /> <span /> <span /> </div> ); } function Sidebar({ page, projectCount, sampleMode, expert, onNavigate, mobileOpen, onClose }: { page: Page; projectCount: number; sampleMode: boolean; expert: boolean; onNavigate: (page: Page) => void; mobileOpen: boolean; onClose: () => void; }) {
  ...
  return ( <>
    ...
    <aside ref={drawerRef} className={`sidebar ${mobileOpen ? "is-open" : ""}`} aria-label="Primary navigation" role={mobileOpen ? "dialog" : undefined} aria-modal={mobileOpen ? true : undefined} tabIndex={mobileOpen ? -1 : undefined} onKeyDown={keepMobileFocusInside}>
      <div className="brand-lockup"><BrandMark /><div><div className="wordmark">BenchLedger</div><div className="brand-caption">Project control</div></div><button ref={closeButtonRef} className="icon-button sidebar-close" aria-label="Close navigation" onClick={closeAndRestoreFocus}><Icon name="close" size={18} /></button></div>
      <div className="workspace-identity" aria-label="Current workspace"><span className="workspace-avatar">W</span><span><strong>Current workspace</strong><small>{sampleMode ? "Sample records" : "Private workspace"}</small></span></div>
...
function PageHeader({ eyebrow, title, description, action, onAction, actionIcon = "plus", children, collapseDescription = false }: { eyebrow: string; title: string; description: string; collapseDescription?: boolean; action?: string | undefined; onAction?: ((event: React.MouseEvent<HTMLButtonElement>) => void) | undefined; actionIcon?: Parameters<typeof Icon>[0]["name"]; children?: ReactNode; }) {
  return ( <div className="page-header"><div><div className="eyebrow">{eyebrow}</div><h1>{title}</h1>{collapseDescription ? <details className="project-description-disclosure"><summary>Project notes</summary><p>{description}</p></details> : <p>{description}</p>}</div><div className="header-actions">{children}{" "} {action && ( <button className="button button-primary" onClick={onAction}><Icon name={actionIcon} size={17} />{action}</button> )}</div></div> );
}
...
function ProjectPage({ pcbSupported, assemblySupported, onImportBom, homeTask, project, projects, projectView, archivedProjectCount, items, offers, tab, expert, sampleMode, reconciliationSupported, makerWorkflowsSupported, onTabChange, onSelectProject, onProjectViewChange, onOpenItem, onNavigate, onToast, onNewProject, onArchive, onRestore, onRemove, onNewRevision, onEditBuildApproach, onRetrySetup, onAddBom, onResolveBomRole, onUpload, onReadReconciliation, onSaveReconciliation, onCommitReconciliation, onRefreshWorkspace, onListInspections, onReadInspection, onPreviewInspection, onConfirmInspection }: {
  ...
  return ( <InspectionContext.Provider value={inspectionContext}><>
    <PageHeader eyebrow="Project" title={project.name} description={project.description} collapseDescription={tab === "assembly" || tab === "pcb"} action={project.status === "archived" ? undefined : "New revision"} onAction={project.status === "archived" ? undefined : onNewRevision}><div className="project-view-switch" role="group" aria-label="Project view"><button type="button" aria-pressed={projectView === "active"} className={projectView === "active" ? "is-active" : ""} onClick={() => onProjectViewChange("active")}> {" "}Active projects{" "} </button><button type="button" aria-pressed={projectView === "archived"} className={projectView === "archived" ? "is-active" : ""} onClick={() => onProjectViewChange("archived")}> {" "}Archived ({archivedProjectCount}){" "} </button></div><select className="project-select" aria-label="Choose project" value={project.id} onChange={(event) => onSelectProject(event.target.value)}>{projects.map((candidate) => ( <option key={candidate.id} value={candidate.id}>{candidate.name}</option>))}</select><button type="button" className="button button-quiet" onClick={onNewProject}> {" "}New project{" "} </button><button type="button" className="button button-danger" onClick={() => setRemoveConfirmationOpen(true)}><Icon name="trash" size={16} /> Delete project</button></PageHeader>
