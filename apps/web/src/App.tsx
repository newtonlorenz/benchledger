...
import { artifactIdentityLabel, artifactScopeChoices, artifactScopeIdentity, artifactScopeKey, defaultArtifactScope, filterArtifactsForScope } from "./artifact-scope";
...
function App() {
  ...
  const uploadArtifact = async (projectId: string, file: File, role: string, target?: ArtifactUploadTarget) => {
    try {
      ...
      setProjects((current) => current.map((candidate) => candidate.id === project.id ? project : candidate));
      setToast(`${file.name} was uploaded to ${target ? artifactScopeIdentity(target) : `Project · ${project.serverRevisionId ?? project.currentRevision}`}.`);
    } catch (error: unknown) {
...
function ConnectionScreen({ state, error, demoAvailable, onLogin, onRetry, onSample }: { state: Exclude<ConnectionState, "loading" | "ready" | "sample">; error: ApiError | undefined; demoAvailable: boolean; onLogin: (password: string) => Promise<void>; onRetry: () => void; onSample: () => void }) {
  ...
  return <main className="connection-screen"><section className="connection-card" aria-labelledby="connection-title"><div className="loading-brand"><div className="brand-mark" aria-hidden="true"><span /><span /><span /></div><span>BenchLedger · private workspace</span></div><div className="connection-state-icon"><Icon name={isAuth ? "info" : isOffline ? "link" : "warning"} size={22} /></div><h1 id="connection-title">{title}</h1><p className="connection-description">{description}</p>{detail && <p className="connection-detail" role="alert">{detail}</p>}{(isAuth || state === "error") && <form className="login-form" onSubmit={submit} noValidate><label className="form-field" htmlFor="workspace-password"><span>Workspace password</span><input id="workspace-password" type="password" autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} aria-describedby={formError ? "workspace-password-error" : undefined} aria-invalid={Boolean(formError)} autoFocus /></label>{formError && <p id="workspace-password-error" className="form-error" role="alert">{formError}</p>}<button className="button button-primary login-submit" type="submit" disabled={submitting}>{submitting ? "Signing in…" : "Sign in"}<Icon name="arrow-right" size={16} /></button></form>}{!isAuth && <button className="button button-secondary connection-retry" onClick={onRetry}><Icon name="refresh" size={16} /> Try again</button>}{demoAvailable && <div className="sample-choice"><span>Sample workspace</span><button className="text-button" onClick={onSample}>Open sample workspace <Icon name="arrow-right" size={15} /></button><small>Sample records are synthetic. BenchLedger does not mix them with private records.</small></div>}</section></main>;
}
...
export function ProjectExpertContext({ project }: { project: Project }) {
  ...
  return <div className="detail-grid">
    {workItems.length ? workItems.map((item) => {
      ...
      return <div key={item.id}><span>Work item</span><code>{item.name} · {item.id} · {revisionId ?? "No current revision"}</code></div>;
...
export function BomLineRow({ line, expert, onOpenItem }: { line: BomLineStatus; expert: boolean; onOpenItem: (id: string) => void }) {
  ...
  return <div className={`bom-row bom-${line.state}`}><div className="bom-main"><span className={`bom-state-mark mark-${display.tone}`} aria-hidden="true">{display.tone === "good" ? "✓" : display.tone === "bad" ? "!" : display.tone === "warn" ? "?" : "–"}</span><div><strong>{line.line.label}</strong><span>{line.line.note ?? `${formatQuantity(required, unit)} required`}</span>{line.missingDecisions?.length ? <small className="bom-missing-decisions">Decide: {line.missingDecisions.join(", ")}</small> : null}</div></div><div className="bom-quantity"><strong>{line.supplied > 0 ? `${formatQuantity(line.supplied, unit)} / ` : ""}{formatQuantity(required, unit)}</strong>{line.remaining > 0 && <small>{formatQuantity(line.remaining, unit)} remaining</small>}</div><div className="bom-match">{line.item ? <button className="match-link" onClick={() => onOpenItem(line.item!.id)}><span>{line.item.name}</span><Icon name="arrow-up-right" size={13} /></button> : <span className="match-none">No matching stock</span>}<span className={`status-pill tone-${display.tone}`}><span className="status-symbol" aria-hidden="true">{display.tone === "good" ? "✓" : display.tone === "bad" ? "!" : display.tone === "warn" ? "?" : "–"}</span>{display.label}</span>{line.decision === "decide" && <small className="bom-decision-label">Decide before source</small>}</div>{expert && <details className="bom-expert"><summary aria-label={`Show evidence for ${line.line.label}`}><Icon name="chevron-down" size={16} /></summary><div><span>Line ID</span><p>{line.line.id}</p><span>Line version</span><p>{line.line.version}</p><span>Canonical unit</span><p>{line.line.serverUnit ?? line.line.unit}</p><span>Match reason</span><p>{line.item ? `${line.item.variant} matches the requested category. Compatibility is based on the recorded project constraint.` : "No exact variant has been recorded in the workspace."}</p>{line.item?.dimensions && <span>Recorded dimensions: {formatDimensions(line.item.dimensions)}</span>}{reasons.length > 0 && <><span>Canonical gap reasons</span><ul>{reasons.map((reason) => <li key={reason}>{reason}</li>)}</ul></>}{diagnostics.length > 0 && <><span>Unit diagnostics</span><ul>{diagnostics.map((diagnostic) => <li key={diagnostic}>{diagnostic}</li>)}</ul></>}{candidates.length > 0 && <><span>Candidate evidence</span><ul>{candidates.map((candidate) => <li key={`${candidate.itemId}-${candidate.relationship}`}>{itemFor(candidate.itemId)?.name ?? candidate.itemId}: {candidate.reason}</li>)}</ul></>}{alternatives.length > 0 && <><span>Structured alternatives</span><ul>{alternatives.map((alternative) => <li key={alternative.itemId}>{alternative.itemId}{alternative.compatible ? ` · ${alternative.compatible}` : ""}{alternative.reason ? ` · ${alternative.reason}` : ""}{alternative.quantityConversion ? ` · 1 ${alternative.quantityConversion.inventory.unit} = ${alternative.quantityConversion.requirement.quantity} ${alternative.quantityConversion.requirement.unit}; ${alternative.quantityConversion.evidence.basis}, observed ${alternative.quantityConversion.evidence.observedAt.slice(0, 10)}` : ""}</li>)}</ul></>}</div></details>}</div>;
}
...
export function ProjectFiles({ project, expert, sampleMode, onUpload, archived = false }: { project: Project; expert: boolean; sampleMode: boolean; onUpload: (file: File, role: string, target?: ArtifactUploadTarget) => Promise<void>; archived?: boolean }) {
  ...
  const processFiles = async (selected: File[]) => {
    ...
    const targetLabel = artifactScopeIdentity(frozenTarget);
  ...
  const scopeDescription = scope.kind === "all"
  ...
  return <section className="surface files-section">
    ...
    {visibleArtifacts.length ? <div className="table-scroll"><table className="data-table files-table"><caption className="sr-only">Artifacts in {artifactScopeIdentity(scope)}</caption><thead><tr><th scope="col">File</th><th scope="col">Role</th><th scope="col">Scope</th><th scope="col">Revision</th><th scope="col">Updated</th><th scope="col">State</th>{expert && <th scope="col">SHA-256</th>}</tr></thead><tbody>{visibleArtifacts.map((file) => <tr key={file.id}><td><span className="file-name"><span className={`file-type type-${file.role.toLowerCase().replaceAll(" ", "-")}`}><Icon name={file.role === "Validation" ? "clipboard" : file.role === "Editable CAD" ? "code" : "file"} size={15} /></span><span><strong>{file.name}</strong><small>{file.size}{file.machine ? ` · ${file.machine}` : ""}</small></span></span></td><td>{file.role}</td><td className="file-scope-cell">{artifactIdentityLabel(file)}</td><td><span className="revision-tag">{file.revision}</span></td><td>{file.updated}</td><td><span className={`file-state state-${file.status}`}>{file.status === "candidate" ? "Candidate" : file.status === "validated" ? "Validated" : "Superseded"}</span></td>{expert && <td><code className="hash-cell">{file.hash}</code></td>}</tr>)}</tbody></table></div> : <div className="files-empty"><Icon name="folder" size={20} /><strong>{scope.kind === "all" ? "No files in this workspace yet." : "No files in this revision yet."}</strong><span>{scope.kind === "all" ? "Legacy and unbound files will appear here when they are retained by the service." : "Add the editable source or first export when you have one."}</span></div>}
    {expert && <details className="expert-detail file-manifest-detail"><summary>Show manifest details</summary><div className="manifest-grid"><span>Binding</span><strong>{bindingLabel}</strong><span>Scope</span><strong>{artifactScopeIdentity(scope)}</strong><span>Retention</span><strong>Older revision files remain auditable when the service records them.</strong><span>Preview</span><strong>Browser-safe text and image previews only.</strong></div></details>}
  </section>;
}
...
function InventoryDrawer({ item, categories, categoriesLoading, categoriesError, expert, onClose, onCount, onCommission, onUpdate }: { item: InventoryItem; categories: readonly ManagedInventoryCategory[]; categoriesLoading: boolean; categoriesError?: string | undefined; expert: boolean; onClose: () => void; onCount: (id: string, quantity: number) => Promise<InventoryItem>; onCommission: (id: string, input: InventoryCommissionInput, expectedVersion: number) => Promise<InventoryItem>; onUpdate: (id: string, input: Partial<InventoryUpdateInput>, expectedVersion?: number) => Promise<InventoryItem> }) {
  ...
  const submitCount = async () => {
    ...
    try {
      ...
      setCountSaved(`Saved ${formatQuantity(result.quantity, result.unit)} as the verified on-hand quantity.`);
    } catch (error: unknown) {
