...
import { InventorySplitter, readInventoryLayout, writeInventoryLayout, defaultInventoryLayout, inventoryOptionalColumns } from "./inventory-layout";
import type { InventoryLayout, InventoryOptionalColumn } from "./inventory-layout";
import { InventoryInspector, InventoryAiCopy } from "./inventory-inspector";
import { parseInventoryView, stockViewOptions, readSavedInventoryViews, saveInventoryViews, inventoryStockLabel, webInventoryAssessment } from "./inventory-workspace-state";
import type { InventoryViewState, SavedInventoryView } from "./inventory-workspace-state";
...
import { useAppearance, AppearanceControl } from "./workspace-controls";
...
import { ProjectEditingContext, ProjectManagementBar, RequirementEditAction, RemovedRequirements, RequirementEditForm, ProjectEditForm, matchesRequirementFilter } from "./project-editing";
import type { RequirementFilter } from "./project-editing";
import type { BomLine } from "./domain";
import { createContext, useContext, useEffect, useId, useLayoutEffect, useRef, useState } from "react";
import type { ChangeEvent, FormEvent, ReactNode } from "react";
import type * as React from "react";
import { ApiError, createSampleWorkspaceAdapter, createWorkspaceAdapter, fetchArtifactDownload, MAX_INVENTORY_SEARCH_LENGTH } from "./api";
...
type ProjectTab = "pcb" | "assembly" | "plan" | "files" | "offers" | "build" | "reconciliation";
...
type ProjectCreateOutcome = "created" | "failed" | "ambiguous" | { error: string };
...
function readInventoryUrlState(): InventoryViewState {
  const params = new URLSearchParams(typeof window === "undefined" ? "" : window.location.search);
  return parseInventoryView({ search: params.get("q"), categoryNodeId: params.get("unassigned") === "true" ? UNASSIGNED_CATEGORY_FILTER : params.get("categoryNodeId"), kind: params.get("kind"), evidence: params.get("evidence"), availability: params.get("available") === "true" ? "available" : params.get("available") === "false" ? "unavailable" : "All", stockView: params.get("stockView"), sort: params.get("sort"), location: params.get("location") });
...
function readNavigationUrlState(): { page: Page; projectId?: string; tab: ProjectTab; projectView?: ProjectView; } {
  ...
  const page = parts[0];
  if (page === "projects") {
    const tab = (["plan", "files", "offers", "build", "assembly", "pcb", "reconciliation"] as ProjectTab[]).includes(parts[2] as ProjectTab) ? (parts[2] as ProjectTab) : "plan";
  ...
  capabilities: { label: "For agents", icon: "code" },
...
export function formatSourceReadyMessage(count: number): string {
  ...
  const applyProjectCorrection = (project: Project, message: string) => {
    setProjects((current) => current.map((entry) => entry.id === project.id ? project : entry));
    setToast(project.readinessUnavailable ? `${message} Stock status could not refresh; reload before sourcing.` : message, project.readinessUnavailable ? "error" : "success");
  };
  ...
  const [inventoryCategoryNodeId, setInventoryCategoryNodeId] = useState(() => readInventoryUrlState().categoryNodeId);
  ...
  useEffect(() => {
    const commands = (event: KeyboardEvent) => {
      if ((event.metaKey || event.ctrlKey) && event.shiftKey && event.key.toLowerCase() === "k" && !event.isComposing) {
        event.preventDefault(); if (!overlayOpen && !mobileNav) setCommandsOpen(true);
      }
    };
    window.addEventListener("keydown", commands); return () => window.removeEventListener("keydown", commands);
  }, [overlayOpen, mobileNav]);
  ...
  }, [page, selectedProjectId, projectTab, projectView, mobileNav]); useEffect(() => {
  ...
  const changeProjectView = (view: "active" | "archived") => navigationGuard.registry.request(() => { pendingMainFocusRef.current = true; setPage("projects"); setMobileNav(false); const nextProjectId = (view === "archived" ? archivedProjects : projects)[0]?.id ?? ""; setProjectView(view); if (nextProjectId) openProject(nextProjectId, projectTab); else { recordNavigation("projects", "", projectTab); setSelectedProjectId(""); } });
 const launchInventorySearch = () => { if (page === "inventory") { if (mainFocusFrameRef.current !== undefined) { window.cancelAnimationFrame(mainFocusFrameRef.current); mainFocusFrameRef.current = undefined; } searchInputRef.current?.focus(); return; } searchHandoffRef.current = true; navigate("inventory"); }; const openNewProject = (event: React.MouseEvent<HTMLButtonElement>) => { newProjectTriggerRef.current = event.currentTarget; setShowNewProject(true); }; const closeNewProject = () => { setShowNewProject(false); const trigger = newProjectTriggerRef.current; if (trigger) window.setTimeout(() => trigger.focus(), 32); }; const openNewPrinter = () => { setReplacementFor(undefined); setShowNewItem(true); }; const openNewPrinterDetails = (item: InventoryItem) => { setReplacementFor(item); setShowNewItem(true); }; const closeNewItem = () => { catalogSearchSequence.current += 1; setCatalogQuery(""); setCatalogProducts([]); setShowNewItem(false); setReplacementFor(undefined); }; const retryConnection = () => { setConnectionError(undefined); setReloadNonce((current) => current + 1); }; const refreshWorkspace = async (): Promise<boolean> => { try { const snapshot = await adapter.loadWorkspace();
      ...
      setSelectedProjectId((current) => current || (snapshot.projects[0] ?? archived[0])?.id || "");
  ...
  const addBomLine = async (input: BomInput): Promise<boolean | { error: string }> => {
    if (!selectedProject) return false;
    try {
      const project = await adapter.createBomLine(selectedProject.id, input);
      setProjects((current) => current.map((candidate) => candidate.id === project.id ? project : candidate));
      setShowAddBom(false);
      setToast(project.readinessUnavailable ? `${input.name} was saved. Stock status could not refresh; reload before sourcing.` : `${input.name} was added to ${project.currentRevision}.`, project.readinessUnavailable ? "error" : "success");
      return true;
    } catch (error: unknown) {
  ...
  const openHomeTask = (task: HomeTask) => {
    setProjectView("active");
    openProject(task.projectId, task.kind === "source" ? "offers" : task.kind === "files" ? "files" : task.kind === "review" ? "build" : "plan");
    if (task.kind === "setup") setShowEditBuildApproach(true);
    else if (task.kind === "requirements") setShowAddBom(true);
    else if (task.kind === "refresh") { void refreshWorkspace(); }
    else if (task.kind === "check" || task.kind === "decide") { pendingMainFocusRef.current = false; setHomeTask({ ...task, request: Date.now() }); }
  };
  const commandItems: WorkspaceCommand[] = [
    ...(["overview", "inventory", "projects", "settings", "capabilities"] as Page[]).map((target) => ({ id: `page:${target}`, label: pageCopy[target].label, detail: target === "capabilities" ? "Read API and agent instructions" : `Open ${pageCopy[target].label.toLowerCase()}`, group: "Navigation" as const, icon: pageCopy[target].icon, run: () => navigate(target) })),
    { id: "new-project", label: "New project", detail: "Add a project and its first revision", group: "Actions", icon: "plus", run: () => setShowNewProject(true) },
    { id: "add-item", label: "Add inventory item", detail: "Record a part, material or tool", group: "Actions", icon: "box", run: () => { setReplacementFor(undefined); setShowNewItem(true); } },
    ...projects.map((project) => ({ id: `project:${project.id}`, label: project.name, detail: project.currentRevision, group: "Projects" as const, icon: "folder" as const, run: () => { setProjectView("active"); openProject(project.id); } })),
    ...items.map((item) => ({ id: `item:${item.id}`, label: item.name, detail: [item.variant, item.location].filter(Boolean).join(" · "), group: "Loaded inventory" as const, icon: "box" as const, run: () => setSelectedItemId(item.id) }))
  ];
  return (
    <UnsavedWorkContext.Provider value={navigationGuard.registry}><div className="app-shell">
      <a className="skip-link" inert={overlayOpen || mobileNav || undefined} aria-hidden={overlayOpen || mobileNav || undefined} href="#main-content" onClick={(event) => { event.preventDefault(); mainRef.current?.focus(); }}>Skip to workspace</a>
      <div className="app-background" aria-hidden={overlayOpen ? true : undefined} inert={overlayOpen || undefined}>
        <Sidebar page={page} projectCount={projects.length} projectNavigation={page === "inventory" ? <InventoryNavigator categories={categories} loading={categoriesLoading} error={categoriesError} selectedId={inventoryCategoryNodeId} onSelect={(id) => { setInventoryCategoryNodeId(id); setMobileNav(false); window.requestAnimationFrame(() => mainRef.current?.focus()); }} onManage={() => navigate("settings")} /> : <ProjectNavigator projects={visibleProjects} selectedId={page === "projects" ? selectedProjectId : undefined} view={projectView} archivedCount={archivedProjects.length} onSelect={selectProject} onViewChange={changeProjectView} />} sampleMode={sampleMode} expert={expert} onNavigate={navigate} mobileOpen={mobileNav} onClose={() => setMobileNav(false)} />
        <div className="app-main" aria-hidden={mobileNav ? true : undefined} inert={mobileNav || undefined}>
          <header className="topbar">
            <button className="icon-button desktop-menu-button" type="button" aria-label={appearance.value.collapsed ? "Expand navigation" : "Collapse navigation"} aria-expanded={!appearance.value.collapsed} title={appearance.value.collapsed ? "Expand navigation" : "Collapse navigation"} onClick={() => appearance.change({ collapsed: !appearance.value.collapsed })}><Icon name="panel-left" size={18} /></button>
            <button className="icon-button mobile-menu-button" aria-label="Open navigation" onClick={() => setMobileNav(true)}>
              <Icon name="menu" size={21} />
            </button>
            <div className="breadcrumb"><span>{pageCopy[page].label}</span>{page === "projects" && selectedProject && <><Icon name="chevron-right" size={14} /><strong>{selectedProject.name}</strong><span className="toolbar-revision">{selectedProject.currentRevision}</span></>}</div>
            <div className="topbar-actions">
              <button type="button" className="command-launcher" aria-label="Open workspace commands" title="Workspace commands (Control or Command + Shift + K)" onClick={() => setCommandsOpen(true)}><Icon name="command" size={16} /><span>Commands</span><kbd>⇧ K</kbd></button>
              <AppearanceControl value={appearance.value} onChange={appearance.change} />
              {page !== "inventory" && ( <button ref={searchLauncherRef} type="button" className="global-search" onClick={launchInventorySearch} aria-label="Search inventory">
                ...
                <kbd>⌘ K</kbd>
              </button> )} <button className="icon-button" aria-label="Open workspace settings" onClick={() => navigate("settings")}> <Icon name="settings" size={19} /> </button>
            </div>
          </header>
          ...
          <main ref={mainRef} className="content" id="main-content" tabIndex={-1} >
            {page === "overview" && ( <OverviewPage onTask={openHomeTask} onRefresh={refreshWorkspace} onImport={sampleMode ? undefined : () => setShowGuidedSetup(true)} onAddItem={() => { setReplacementFor(undefined); setShowNewItem(true); }} items={items} projects={projects} expert={expert} sampleMode={sampleMode} onNavigate={navigate} onOpenProject={(id, tab) => { setProjectView("active"); openProject(id, tab); }} onSelectItem={setSelectedItemId} onNewProject={openNewProject} onAddPrinter={openNewPrinter} /> )}{" "}
            {page === "inventory" && ( <InventoryPage key={sampleMode ? "sample" : "workspace"} categoryNodeId={inventoryCategoryNodeId} setCategoryNodeId={setInventoryCategoryNodeId} sampleMode={sampleMode} adapter={adapter} categories={categories} expert={expert} search={search} searchInputRef={searchInputRef} refreshKey={inventoryRefreshNonce} bulkSelectionResetKey={bulkSelectionResetNonce} onSearch={(value) => setSearch(value.slice(0, MAX_INVENTORY_SEARCH_LENGTH))} onSessionExpired={handleSessionExpiry} onPageItems={(pageItems) => setItems((current) => { const byId = new Map(current.map((item) => [item.id, item] as const)); pageItems.forEach((item) => byId.set(item.id, item)); return [...byId.values()]; })} onSelectItem={setSelectedItemId} onNewItem={() => { setReplacementFor(undefined); setShowNewItem(true); }} onBulkSelectionChange={(selection, onResult) => setBulkInventorySelection(selection.length ? { items: [...selection], onResult } : undefined)} /> )}{" "}
            {page === "projects" && selectedProject && ( <ProjectEditingContext.Provider value={{ project: selectedProject, refreshProject: async () => { const refreshed = await refreshWorkspace(); if (refreshed) { setProjectRefreshNonce((value) => value + 1); setToast("Project refreshed from the workspace."); } return refreshed; }, editRequirement: (line) => setEditingRequirement({ projectId: selectedProject.id, line }), editProject: () => setEditingProject(selectedProject), listRemoved: () => adapter.listRetiredBomLines(selectedProject.id), restore: async (line) => { applyProjectCorrection(await adapter.restoreBomLine(selectedProject.id, line.id, line.version), "Requirement restored. No stock was reserved."); } }}><ProjectPage pcbSupported={serviceCapabilities.includes("pcb.read")} assemblySupported={serviceCapabilities.includes("assembly.read")} key={projectRefreshNonce} onImportBom={() => setShowBomImport(true)} homeTask={homeTask?.projectId === selectedProject.id ? homeTask : undefined} makerWorkflowsSupported={!sampleMode && serviceCapabilities.includes("maker_workflows.read")} project={selectedProject} projects={visibleProjects} projectView={projectView} archivedProjectCount={archivedProjects.length} items={items} offers={offers} tab={projectTab} expert={expert} sampleMode={sampleMode} reconciliationSupported={ serviceCapabilities.includes("reconciliation.read") && serviceCapabilities.includes("reconciliation.write") } onTabChange={selectProjectTab} onSelectProject={selectProject} onProjectViewChange={changeProjectView} onOpenItem={setSelectedItemId} onNavigate={navigate} onToast={setToast} onNewProject={openNewProject} onArchive={archiveProject} onRestore={restoreProject} onRemove={removeProject} onNewRevision={() => navigationGuard.registry.request(() => setShowNewRevision(true))} onEditBuildApproach={() => setShowEditBuildApproach(true)} onRetrySetup={pendingRevisionSetup?.projectId === selectedProject.id && pendingRevisionSetup.revisionId === selectedProject.serverRevisionId ? retryRevisionSetup : undefined} onAddBom={() => setShowAddBom(true)} onResolveBomRole={resolveBomLineRole} onUpload={uploadArtifact} onReadReconciliation={adapter.readReconciliation} onSaveReconciliation={adapter.saveReconciliationDraft} onCommitReconciliation={adapter.commitReconciliation} onRefreshWorkspace={refreshWorkspace} onListInspections={adapter.listInspections} onReadInspection={adapter.readInspection} onPreviewInspection={adapter.previewInspectionCompletion} onConfirmInspection={adapter.commitInspectionCompletion} /></ProjectEditingContext.Provider> )}{" "}
            {page === "projects" && !selectedProject && selectedProjectId && <section className="surface workflow-unavailable"><h1>Project unavailable</h1><p>This project is not in the loaded workspace. It may have been removed or be unavailable to this account. No other project was opened.</p><button type="button" className="button button-primary" onClick={() => navigate("overview")}>Open project register</button><button type="button" className="button button-quiet" onClick={() => { void refreshWorkspace(); }}>Retry project lookup</button></section>}{page === "projects" && !selectedProject && !selectedProjectId && ( <section><EmptyState icon="folder" title={projectView === "archived" ? "No archived projects" : "No projects yet"} description={projectView === "archived" ? "Archived projects will appear here with their retained history." : "Enter a project name and goal. Then add requirements and files."} {...(projectView === "active" ? { action: "Create first project", onAction: () => setShowNewProject(true) } : {})} /></section> )}{" "}
      ...
      {selectedItem && ( <InventoryDrawer suspended={Boolean(navigationGuard.pending)} sampleMode={sampleMode} item={selectedItem} items={items} categories={categories} categoriesLoading={categoriesLoading} categoriesError={categoriesError} expert={expert} onClose={() => navigationGuard.registry.request(() => setSelectedItemId(undefined))} onCount={recordCount} onCommission={commissionInventoryItem} onUpdate={updateInventoryItem} onDelete={async (item) => {
        ...
        setSelectedItemId(undefined);
        setBulkInventorySelection(undefined);
        setBulkSelectionResetNonce((value) => value + 1);
      ...
      {editingProject && <Dialog title="Edit project" onClose={() => { if (!correctionBusy) setEditingProject(undefined); }}><ProjectEditForm project={editingProject} onBusy={setCorrectionBusy} onClose={() => setEditingProject(undefined)} onSave={async (input) => { if (editingProject.version === undefined) throw new ApiError("Reload this project to obtain its current version before editing.", { kind: "validation", status: 409 }); applyProjectCorrection(await adapter.updateProject(editingProject.id, input, editingProject.version), "Project updated. Stock and validation evidence are unchanged."); setEditingProject(undefined); }} /></Dialog>}
      {editingRequirement && <Dialog title="Edit requirement" onClose={() => { if (!correctionBusy) setEditingRequirement(undefined); }}><RequirementEditForm line={editingRequirement.line} items={items} onBusy={setCorrectionBusy} onClose={() => setEditingRequirement(undefined)} onSave={async (input) => { applyProjectCorrection(await adapter.updateBomLine(editingRequirement.projectId, editingRequirement.line.id, input, editingRequirement.line.version), "Requirement saved."); setEditingRequirement(undefined); }} onRetire={async () => { applyProjectCorrection(await adapter.retireBomLine(editingRequirement.projectId, editingRequirement.line.id, editingRequirement.line.version), "Requirement removed from the plan. Its history is retained."); setEditingRequirement(undefined); }} /></Dialog>}
    ...
    </div></UnsavedWorkContext.Provider>
  );
}

function BrandMark() { return ( <div className="brand-mark" aria-hidden="true"> <span /> <span /> <span /> </div> ); } function Sidebar({ page, projectCount, sampleMode, expert, onNavigate, mobileOpen, onClose, projectNavigation }: { page: Page; projectCount: number; projectNavigation: ReactNode; sampleMode: boolean; expert: boolean; onNavigate: (page: Page) => void; mobileOpen: boolean; onClose: () => void; }) {
  ...
  return ( <>
    ...
    <aside ref={drawerRef} className={`sidebar ${mobileOpen ? "is-open" : ""}`} aria-label="Primary navigation" role={mobileOpen ? "dialog" : undefined} aria-modal={mobileOpen ? true : undefined} tabIndex={mobileOpen ? -1 : undefined} onKeyDown={keepMobileFocusInside}>
      <div className="brand-lockup"><BrandMark /><div><div className="wordmark">BenchLedger</div></div><button ref={closeButtonRef} className="icon-button sidebar-close" aria-label="Close navigation" onClick={closeAndRestoreFocus}><Icon name="close" size={18} /></button></div>
      <div className="workspace-identity" aria-label="Current workspace"><span className="online-dot" /><span>{sampleMode ? "Sample workspace" : "Private workspace"}</span></div>
      <nav className="nav-list">
        {(["overview", "inventory", "projects"] as Page[]).map((entry) => ( <button key={entry} className={`nav-item ${page === entry ? "is-active" : ""}`} title={pageCopy[entry].label} aria-current={page === entry ? "page" : undefined} data-label={pageCopy[entry].label} onClick={() => onNavigate(entry)}><Icon name={pageCopy[entry].icon} size={18} /><span>{pageCopy[entry].label}</span>{entry === "projects" && ( <span className="nav-count">{projectCount}</span> )}</button>))}{" "} {expert && ( <> <span className="nav-label nav-label-agent">Agent access</span>
        <button className={`nav-item ${page === "capabilities" ? "is-active" : ""}`} title="For agents" aria-current={page === "capabilities" ? "page" : undefined} data-label="For agents" onClick={() => onNavigate("capabilities")}><Icon name="code" size={18} /><span>For agents</span><span className="status-dot" /></button>
      </> )} </nav>
      {projectNavigation}<div className="sidebar-bottom"><button className={`nav-item ${page === "settings" ? "is-active" : ""}`} title="Settings" aria-current={page === "settings" ? "page" : undefined} data-label="Settings" onClick={() => onNavigate("settings")}><Icon name="settings" size={18} /><span>Settings</span></button><div className="connection-note"><span className="online-dot" /><span>{sampleMode ? "Sample workspace" : "Connected"}</span></div></div>
    </aside>
  </> );
}
...
function PageHeader({ title, description, action, onAction, actionIcon = "plus", children, collapseDescription = false }: { eyebrow: string; title: string; description: string; collapseDescription?: boolean; action?: string | undefined; onAction?: ((event: React.MouseEvent<HTMLButtonElement>) => void) | undefined; actionIcon?: Parameters<typeof Icon>[0]["name"]; children?: ReactNode; }) {
  return ( <div className="page-header"><div><h1>{title}</h1>{collapseDescription ? <details className="project-description-disclosure"><summary>Project notes</summary><p>{description}</p></details> : <p>{description}</p>}</div><div className="header-actions">{children}{" "} {action && ( <button className="button button-primary" onClick={onAction}><Icon name={actionIcon} size={17} />{action}</button> )}</div></div> );
}
...
function projectFabricationRoute(project: Project): FabricationRoute { const plannedPrinter = (project as NullablePrinterProject).intendedPrinterItemId; if (project.fabricationRoute) return project.fabricationRoute; if (plannedPrinter !== undefined) return plannedPrinter ? "printed" : "undecided"; return project.buildConfigSnapshot?.printerItemId ? "printed" : "undecided"; } function projectIntendedPrinterId(project: Project): string | undefined { const plannedPrinter = (project as NullablePrinterProject).intendedPrinterItemId; if (plannedPrinter !== undefined) return plannedPrinter ?? undefined; return project.fabricationRoute === undefined ? project.buildConfigSnapshot?.printerItemId : undefined; } export function isUsableOwnedPrinter(item: InventoryItem): boolean { const evidence = item.serverEvidence ?? item.evidence; const retired = (item as InventoryItem & { retired?: boolean }).retired === true || item.tags.some((tag) => tag.toLocaleLowerCase() === "retired");
  ...
  const profileMatchesItem = item.productProfile?.inventoryItemId === item.id && item.productProfile.catalogProductId === item.catalogProduct?.id && item.productProfile.linkState === "confirmed" && (item.productProfile.profileType === "printer_asset" || item.productProfile.printer !== undefined); return ( item.category === "Printers" && !retired && item.unit === "each" && item.unitStatus !== "needs_correction" && item.quantity > 0 && availableQuantity > 0 && (evidence === "physically_counted" || evidence === "commissioned") && item.catalogProduct?.kind === "printer" && isExactProductIdentityComplete(item) && profileMatchesItem ); } export function printerBuildVolumeCopy( item: InventoryItem | undefined ): string | undefined { const volume = item?.catalogProduct?.buildVolumeMm; if ( !volume || ![volume.x, volume.y, volume.z].every((value) => typeof value === "number" && Number.isFinite(value) && value > 0 ) ) return undefined; return `${volume.x} × ${volume.y} × ${volume.z} mm build volume`; } export interface PrintRelatedSignals { requirementCount: number; fileCount: number; hasBuildSetup: boolean; } export function printRelatedSignals( project: Project, items: InventoryItem[] ): PrintRelatedSignals { const printRequirementCount = project.bom.filter((line) => { const item = line.itemId ? items.find((candidate) => candidate.id === line.itemId) : undefined; const itemIsPrintRelated = item?.category === "Filament" || item?.category === "Printers" || item?.kind === "filament" || item?.kind === "printer"; const constrainedKind = line.constraints?.kind; return ( itemIsPrintRelated || constrainedKind === "filament" || constrainedKind === "printer" ); }).length; const printableFileCount = project.artifacts.filter( (artifact) => artifact.role === "STL" || artifact.role === "Build plate" ).length; return { requirementCount: printRequirementCount, fileCount: printableFileCount, hasBuildSetup: project.buildConfigSnapshot !== undefined }; } export function hasCurrentRevisionPrintableArtifact(project: Project): boolean { const currentWorkItemRevisions = new Map( (project.workItems ?? []).flatMap((item) => { const revisionId = item.currentRevisionId ?? item.currentRevision?.id; return revisionId ? [[item.id, revisionId] as const] : []; }) ); return project.artifacts.some((artifact) => { if (artifact.role !== "STL" && artifact.role !== "Build plate") return false; if (artifact.workItemId !== undefined) { return ( artifact.workItemRevisionId !== undefined && currentWorkItemRevisions.get(artifact.workItemId) === artifact.workItemRevisionId ); } return ( project.serverRevisionId !== undefined && artifact.projectRevisionId === project.serverRevisionId ); }); } function printReviewCopy(signals: PrintRelatedSignals): string | undefined { const itemCount = signals.requirementCount + signals.fileCount; if (itemCount > 0) return `Review ${itemCount} print-related item${itemCount === 1 ? "" : "s"} before continuing with this route.`; if (signals.hasBuildSetup) return "A saved print setup remains. Review it before continuing with this route."; return undefined; } export function BuildApproachCard({ project, items, expert, onSelectPrinter, onChoosePrinter, onReviewPrintItems }: { project: Project; items: InventoryItem[]; expert: boolean; onSelectPrinter?: ((id: string) => void) | undefined; onChoosePrinter?: (() => void) | undefined; onReviewPrintItems?: ((target: "plan" | "files") => void) | undefined; }) { const route = projectFabricationRoute(project); const printerId = projectIntendedPrinterId(project); const printer = printerId ? items.find((item) => item.id === printerId) : undefined; const usablePrinter = printer && isUsableOwnedPrinter(printer) ? printer : undefined; const selectedPrinterLabel = usablePrinter ? inventoryCandidateLabel(usablePrinter, items, expert).name : undefined; const routeCopy = route === "ready_made" ? "Use a bought or existing enclosure/part" : route === "none" ? "Electronics / assembly only" : route === "printed" ? (selectedPrinterLabel ?? (printer ? "Printer needs a check" : "No printer selected yet")) : "Choose how this project will be built."; const printSignals = route === "ready_made" || route === "none" ? printRelatedSignals(project, items) : undefined; const reviewCopy = printSignals ? printReviewCopy(printSignals) : undefined; const reviewTarget = printSignals && printSignals.requirementCount === 0 && printSignals.fileCount > 0 ? ("files" as const) : ("plan" as const); const hasPrintableFile = route === "printed" && hasCurrentRevisionPrintableArtifact(project); const detailCopy = route === "printed" ? usablePrinter ? `${printerBuildVolumeCopy(usablePrinter) ?? "Build volume not recorded"}.` : printer ? "This printer is not available as an owned capability until it is physically counted or commissioned." : "Choose or add an owned printer when needed." : route === "ready_made" ? "Next, add the dimensions or requirement for the part you will use." : route === "none" ? "No printer is needed for this approach." : "Select a build approach when the requirements are known."; return ( <section className="surface build-approach-card" aria-label="Build approach" > <div className="build-approach-heading"> <div> <span className="eyebrow">Build approach</span> <h2>{routeCopy}</h2> </div> {route !== "printed" && ( <span className="route-chip">{fabricationRouteLabel(route)}</span> )} </div> <p>{detailCopy}</p> {reviewCopy && ( <div className="build-approach-review" role="note"> <strong>Review print-related items</strong> <span>{reviewCopy}</span> {onReviewPrintItems && ( <button type="button" className="text-button" onClick={() => onReviewPrintItems(reviewTarget)} > {reviewTarget === "files" ? "Open Files" : "Open Plan"} <Icon name="arrow-right" size={14} /> </button> )} </div> )}{" "} {route === "printed" && ( <div className="build-approach-fit"> <strong>Not checked yet.</strong> <span> {hasPrintableFile ? "Build file added. Fit still needs a slicer check." : "Add a printable file to check fit."} </span> </div> )}{" "} {(onChoosePrinter || (route === "printed" && printer && onSelectPrinter)) && ( <div className="build-approach-actions"> {onChoosePrinter && ( <button type="button" className="text-button" onClick={onChoosePrinter} > {" "}
              ...
              Open printer details <Icon name="arrow-up-right" size={14} /> </button> )} </div> )}{" "} {expert && ( <details className="expert-detail build-approach-expert"> <summary>Planning details</summary> <div className="detail-grid"> <div> <span>Fabrication route</span> <code>{route}</code> </div> <div> <span>Intended printer</span> <code>{printerId ?? "Not selected"}</code> </div> <div> <span>Printer build volume</span> <code>{printerBuildVolumeCopy(printer) ?? "Not recorded"}</code> </div> </div> </details> )} </section> ); } export function OverviewPage({ items, projects, expert, sampleMode, onNavigate, onOpenProject, onSelectItem, onNewProject, onAddPrinter, onAddItem, onImport, onTask, onRefresh }: {
  items: InventoryItem[]; projects: Project[]; expert: boolean; sampleMode: boolean;
  onNavigate(page: Page): void; onOpenProject(id: string, tab?: ProjectTab): void; onSelectItem(id: string): void;
  onNewProject(event: React.MouseEvent<HTMLButtonElement>): void; onAddPrinter?: (() => void) | undefined;
  onAddItem?: (() => void) | undefined; onImport?: (() => void) | undefined;
  onTask?: ((task: HomeTask) => void) | undefined; onRefresh?: (() => Promise<boolean>) | undefined;
}) {
  return <><WorkbenchHome projects={projects} items={items} printers={items.filter((item) => item.category === "Printers" && !item.tags.some((tag) => tag.toLowerCase() === "retired"))} isPrinterUsable={isUsableOwnedPrinter} sampleMode={sampleMode}
    onOpen={onOpenProject} onTask={onTask ?? ((task) => onOpenProject(task.projectId, task.kind === "source" ? "offers" : task.kind === "files" ? "files" : "plan"))}
    onNewProject={onNewProject} onAddItem={onAddItem ?? onAddPrinter ?? (() => onNavigate("inventory"))} onImport={onImport}
    onInventory={() => onNavigate("inventory")} onItem={onSelectItem} onRefresh={onRefresh} />
    {expert && <details className="home-technical-details"><summary>Technical project context</summary><p>Open a project to read its current revision and recorded evidence.</p><button type="button" className="text-button workshop-agent-link" onClick={() => onNavigate("capabilities")}>For agents<Icon name="code" size={15} /></button></details>}
  </>;
}
...
function SectionHeading({ title, action, onAction }: { eyebrow: string; title: string; action?: string | undefined; onAction?: (() => void) | undefined; }) {
  return ( <div className="section-heading"><div><h2>{title}</h2></div>{action && ( <button className="text-button" onClick={onAction}>{action}<Icon name="arrow-right" size={14} /></button> )}</div> );
}
...
function InventoryPage({ categoryNodeId, setCategoryNodeId, sampleMode, adapter, categories, expert, search, searchInputRef, refreshKey, bulkSelectionResetKey, onSearch, onSessionExpired, onPageItems, onSelectItem, onNewItem, onBulkSelectionChange }: { categoryNodeId: string; setCategoryNodeId: (id: string) => void; sampleMode: boolean; adapter: WorkspaceAdapter; categories: readonly ManagedInventoryCategory[]; expert: boolean; search: string; searchInputRef: React.RefObject<HTMLInputElement | null>; refreshKey: number; bulkSelectionResetKey: number; onSearch: (value: string) => void; onSessionExpired: (error: unknown) => void; onPageItems: (items: readonly InventoryItem[]) => void; onSelectItem: (id: string) => void; onNewItem: () => void; onBulkSelectionChange: (items: readonly VersionedInventoryItem[], onResult: (result: InventoryBulkUpdateResult) => void) => void; }) {
  const initialUrlState = readInventoryUrlState();
  const [stockView, setStockView] = useState(initialUrlState.stockView);
  const [sort, setSort] = useState(initialUrlState.sort);
  const [location, setLocation] = useState(initialUrlState.location);
  const [inspectedId, setInspectedId] = useState<string>();
  const [layout, setLayout] = useState(() => readInventoryLayout(sampleMode));
  const inspectorOpen = layout.inspectorOpen;
  const updateLayout = (next: InventoryLayout, commit = true) => {
    setLayout(next);
    if (commit && !writeInventoryLayout(next, sampleMode)) setViewNotice("Layout changed for this session. Browser storage is unavailable.");
  };
  const setInspectorOpen = (open: boolean) => updateLayout({ ...layout, inspectorOpen: open });
  const [columnsOpen, setColumnsOpen] = useState(false);
  const [mobileFiltersOpen, setMobileFiltersOpen] = useState(false);
  const [savedViews, setSavedViews] = useState<SavedInventoryView[]>(() => readSavedInventoryViews(sampleMode));
  ...
  const filterKey = JSON.stringify([search, categoryNodeId, kind, evidence, availability, stockView, sort, location]);
  const previousFilterKey = useRef(filterKey); const selectedKindLabel = inventoryKindOptions.find( (option) => option.value === kind )?.label; const advancedFilterSummary = [ location.trim() ? `Location: ${location.trim()}` : undefined, kind !== "All" && selectedKindLabel ? `Item type: ${selectedKindLabel}` : undefined ] .filter((value): value is string => Boolean(value)) .join(" · "); // Capture the selection from the current render before a filter change
  ...
  useEffect(() => {
    ...
    ["q", "categoryNodeId", "unassigned", "kind", "evidence", "available", "stockView", "sort", "location"].forEach((key) => params.delete(key));
    if (stockView !== "all") params.set("stockView", stockView);
  ...
  }, [search, categoryNodeId, kind, evidence, availability, stockView, sort, location]);
  ...
  const currentView: InventoryViewState = { search, categoryNodeId, kind, evidence, availability, stockView, sort, location };
  const applyView = (view: InventoryViewState) => {
    onSearch(view.search); setCategoryNodeId(view.categoryNodeId); setKind(view.kind); setEvidence(view.evidence);
    setAvailability(view.availability); setStockView(view.stockView); setSort(view.sort); setLocation(view.location);
  };
  const clearFilters = () => applyView(parseInventoryView({}));
  const savedViewName = savedViews.find((view) => JSON.stringify(view.filters) === JSON.stringify(parseInventoryView(currentView)))?.name ?? "";
  const saveView = (event: FormEvent) => {
    event.preventDefault();
    const name = viewName.trim();
    if (!name) return;
    if (savedViews.some((view) => view.name === name)) { setViewNotice("That view name already exists. Choose another name."); return; }
    if (savedViews.length >= 12) { setViewNotice("You can save up to 12 views. Remove a saved view first."); return; }
    const next = [...savedViews, { name, filters: parseInventoryView(currentView) }];
    const persisted = saveInventoryViews(next, sampleMode);
    setSavedViews(next); setSaveViewOpen(false); setViewName("");
    setViewNotice(persisted ? `Saved “${name}” in this browser.` : "View available for this session. Browser storage is unavailable.");
  };
  const removeView = () => {
    const next = savedViews.filter((view) => view.name !== savedViewName);
    const persisted = saveInventoryViews(next, sampleMode); setSavedViews(next);
    setViewNotice(persisted ? "Saved view removed. Inventory records are unchanged." : "View removed for this session. Browser storage is unavailable.");
  };
  const inspectedItem = pageItems.find((item) => item.id === inspectedId) ?? pageItems[0];
  const inspectItem = (id: string) => { setInspectedId(id); };
  const inspectedCategory = inspectedItem ? managedInventoryLabel(categories, inspectedItem) : undefined;
  const activeView = stockViewOptions.find((view) => view.value === stockView)!;
  return ( <>
    <PageHeader eyebrow="Inventory" title="Inventory" description="Parts, materials and equipment. Stock you can account for." action="Add item" onAction={onNewItem} />
    <section className={`surface inventory-section inventory-workstation ${inspectorOpen ? "has-inspector" : ""}`} style={{ "--inventory-inspector-width": `${layout.inspectorWidth}px` } as React.CSSProperties}>
      <div className="inventory-view-bar">
        <nav aria-label="Inventory stock views">{stockViewOptions.map((view) => <button key={view.value} aria-current={stockView === view.value ? "page" : undefined} title={view.description} onClick={() => { setStockView(view.value); setAvailability("All"); setEvidence("All"); }}>{view.label}</button>)}</nav>
        <button className="button button-secondary" aria-expanded={inspectorOpen} onClick={() => setInspectorOpen(!inspectorOpen)}><Icon name="sliders" size={14} />Inspector</button>
      </div>
      {stockView !== "all" && <div className="inventory-view-description">{activeView.description}</div>}
      <div className={`inventory-toolbar ${mobileFiltersOpen ? "filters-open" : ""}`} aria-label="Inventory filters">
        ...
        <label className="inventory-sort-control">Sort<select aria-label="Sort inventory" value={sort} onChange={(event) => setSort(event.target.value as typeof sort)}><option value="name">Name A–Z</option><option value="name_desc">Name Z–A</option><option value="location">Location</option></select></label>
        <button type="button" className="button button-secondary inventory-mobile-filters" aria-expanded={mobileFiltersOpen} onClick={() => setMobileFiltersOpen(!mobileFiltersOpen)}><Icon name="filter" size={14} />Filters</button>
        <div className="inventory-filter-grid">
          ...
          </summary> <div className="inventory-more-filter-grid"> <InventoryFilter label="Item type" value={kind} onChange={(value) => setKind(value as InventoryKindQuery | "All")} options={[{ value: "All", label: "All item types" }, ...inventoryKindOptions ]} /><label className="inventory-location-filter">Exact location<input aria-label="Filter inventory by exact location" value={location} maxLength={256} onChange={(event) => setLocation(event.target.value)} placeholder="Any location" /></label> {advancedFilterSummary && ( <button type="button" className="text-button inventory-clear-more" onClick={() => { setKind("All"); setLocation(""); }} > {" "}
                    Clear more filters{" "} </button> )} </div> </details>
        </div>
      </div>
      <div className="inventory-saved-bar">
        <label>Saved view<select aria-label="Saved inventory view" value={savedViewName} onChange={(event) => { const view = savedViews.find((view) => view.name === event.target.value); if (view) applyView(view.filters); }}><option value="">Custom view</option>{savedViews.map((view) => <option key={view.name} value={view.name}>{view.name}</option>)}</select></label>
        <button className="text-button" onClick={() => { setSaveViewOpen(!saveViewOpen); setViewNotice(""); }}>Save view…</button>
        {savedViewName && <button className="text-button" onClick={removeView}>Remove saved view</button>}
        <button className="text-button" onClick={clearFilters}>Reset filters</button>
        <button className="text-button" aria-expanded={columnsOpen} aria-controls="inventory-column-controls" onClick={() => setColumnsOpen(!columnsOpen)}>Columns</button>
        <div className="inventory-result-controls"><div className="inventory-page-status" role="status" aria-live="polite">{loading ? "Loading inventory…" : error ? "Inventory could not be loaded." : loadMoreError ? "Showing the loaded items. More items could not be loaded." : total === undefined ? `Showing ${pageItems.length} items` : `Showing ${pageItems.length} of ${total} items`}</div>
          <button className="icon-button" aria-label="Refresh inventory" onClick={() => setRetryNonce((value) => value + 1)}><Icon name="refresh" size={15} /></button></div>
      </div>
      {columnsOpen && <fieldset id="inventory-column-controls" className="inventory-column-controls"><legend>Additional columns</legend>{inventoryOptionalColumns.map((column) => <label key={column}><input type="checkbox" checked={layout.columns.includes(column)} onChange={() => updateLayout({ ...layout, columns: layout.columns.includes(column) ? layout.columns.filter((value) => value !== column) : [...layout.columns, column] })} />{column === "sku" ? "SKU / part number" : column[0]!.toUpperCase() + column.slice(1)}</label>)}<button type="button" className="text-button" onClick={() => updateLayout({ ...defaultInventoryLayout, columns: [...defaultInventoryLayout.columns] })}>Reset layout</button><small>Item, recorded stock, available stock and status stay visible.</small></fieldset>}
      {saveViewOpen && <form className="inventory-save-form" onSubmit={saveView}><label>View name<input aria-label="Inventory view name" value={viewName} onChange={(event) => setViewName(event.target.value)} maxLength={60} placeholder="e.g. Electronics in drawer A" autoFocus /></label><button className="button button-primary" disabled={!viewName.trim()}>Save view</button><button type="button" className="text-button" onClick={() => setSaveViewOpen(false)}>Cancel</button><small>Filters and sorting are saved in this browser.</small></form>}
      {viewNotice && <p className="inventory-selection-notice" role="status">{viewNotice}</p>}

      {selectedTargets.size > 0 && ( <div className="inventory-selection-bar" aria-label="Bulk inventory selection"><div><strong>{selectedTargets.size} selected of {pageItems.length}{" "} loaded{" "} </strong><span> {" "}Select all applies only to the items currently loaded. You can select up to 100.{" "} </span></div><InventoryAiCopy key={[...selectedTargets.keys()].join("|")} items={pageItems.filter((item) => selectedTargets.has(item.id))} /><button className="button button-secondary" onClick={openBulkEditor}> {" "}Bulk edit<Icon name="sliders" size={16} /></button></div> )}{" "}
      ...
      <div className="inventory-record-bar" aria-label="Selected inventory record"><span>{inspectedItem ? inspectedItem.name : "No item selected"}</span><button type="button" className="button button-secondary" disabled={!inspectedItem || loading} onClick={() => inspectedItem && onSelectItem(inspectedItem.id)}>Open item<kbd>↵</kbd></button><small>Click to select · Double-click to open</small></div>
      <div className="inventory-work-area"><div className="inventory-register">{error ? ( <div className="inventory-load-error" role="alert"><span>{error.message}</span><button className="button button-secondary" onClick={() => setRetryNonce((value) => value + 1)}> {" "}Try again{" "} </button></div> ) : loading && pageItems.length === 0 ? ( <div className="inventory-loading" aria-label="Loading inventory"> {" "}Loading inventory…{" "} </div> ) : pageItems.length ? ( <><InventoryTable sort={sort} onSortChange={setSort} visibleColumns={layout.columns} inspectedId={inspectedItem?.id} onInspectItem={inspectItem} items={pageItems} categories={categories} selectedIds={new Set(selectedTargets.keys())} selectAllRef={selectAllRef} allLoadedSelected={allLoadedSelected} hasUnversionedLoaded={Boolean(unversionedItem)} onToggleAll={toggleAllLoaded} onToggleSelected={toggleSelected} onSelectItem={onSelectItem} />{loadMoreError && ( <div className="inventory-load-error" role="alert"><span>{loadMoreError.message}</span><button className="button button-secondary" onClick={() => { void loadMore(); }}> {" "}Try again{" "} </button></div> )}{" "}{nextCursor && ( <div className="inventory-load-more"><button className="button button-secondary" onClick={() => { void loadMore(); }} disabled={loadingMore} aria-busy={loadingMore}>{loadingMore ? "Loading…" : "Load more"}<Icon name="chevron-right" size={16} /></button></div> )}</> ) : ( <EmptyState icon="search" title="No matching items" description="Change the search text or filters." action="Clear filters" onAction={clearFilters} /> )}</div>{inspectorOpen && <><InventorySplitter width={layout.inspectorWidth} onChange={(width, commit) => updateLayout({ ...layout, inspectorWidth: width }, commit)} /><InventoryInspector item={loading || error ? undefined : inspectedItem} category={inspectedCategory} onOpen={onSelectItem} onClose={() => setInspectorOpen(false)} /></>}</div>
    </section>
  </> );
}
...
} export function InventoryTable({ items, categories, selectedIds, selectAllRef, allLoadedSelected, hasUnversionedLoaded, onToggleAll, onToggleSelected, onSelectItem, inspectedId, onInspectItem, visibleColumns = ["category", "location"], sort = "name", onSortChange }:  { items: InventoryItem[]; categories: readonly ManagedInventoryCategory[]; selectedIds: ReadonlySet<string>; selectAllRef: React.RefObject<HTMLInputElement | null>; allLoadedSelected: boolean; hasUnversionedLoaded: boolean; onToggleAll: () => void; onToggleSelected: (id: string) => void; onSelectItem: (id: string) => void; inspectedId?: string | undefined; onInspectItem?: (id: string) => void; visibleColumns?: readonly InventoryOptionalColumn[]; sort?: InventoryViewState["sort"]; onSortChange?: (sort: InventoryViewState["sort"]) => void; }) {
  const moveInspector = (event: React.KeyboardEvent<HTMLTableRowElement>, index: number) => {
    if (!onInspectItem || (event.target !== event.currentTarget && !(event.target as HTMLElement).closest(".table-item"))) return;
    if (event.key === "Enter" || event.key === "F2") { event.preventDefault(); onSelectItem(items[index]!.id); return; }
    const next = event.key === "ArrowDown" ? Math.min(items.length - 1, index + 1) : event.key === "ArrowUp" ? Math.max(0, index - 1) : event.key === "Home" ? 0 : event.key === "End" ? items.length - 1 : -1;
    if (next < 0) return;
    event.preventDefault(); onInspectItem(items[next]!.id);
    const rows = event.currentTarget.parentElement?.querySelectorAll("tr"); rows?.[next]?.focus();
  };
  return <div className="table-scroll"><table className="data-table inventory-table"><caption className="sr-only">Inventory items. Click to select, double-click to open. Use arrow keys to move and Enter or F2 to open. Checkboxes select records for bulk actions.</caption><thead><tr>
    ...
    <th scope="col" aria-sort={sort === "name" ? "ascending" : sort === "name_desc" ? "descending" : "none"}>{onSortChange ? <button className="inventory-sort-heading" onClick={() => onSortChange(sort === "name" ? "name_desc" : "name")}>Item<Icon name="chevron-down" size={12} className={sort === "name_desc" ? "is-descending" : undefined} /></button> : "Item"}</th>{visibleColumns.includes("category") && <th scope="col" className="inventory-category-column">Category</th>}<th scope="col">Recorded</th><th scope="col">Available</th>{visibleColumns.includes("reserved") && <th scope="col">Reserved</th>}<th scope="col">Status</th>{visibleColumns.includes("location") && <th scope="col" className="inventory-location-column" aria-sort={sort === "location" ? "ascending" : "none"}>{onSortChange ? <button className="inventory-sort-heading" onClick={() => onSortChange("location")}>Location<Icon name="chevron-down" size={12} /></button> : "Location"}</th>}{visibleColumns.includes("sku") && <th scope="col">SKU / part number</th>}<th scope="col"><span className="sr-only">Open</span></th>
  </tr></thead><tbody>{items.map((item, index) => {
    const categoryLabel = managedInventoryLabel(categories, item);
    const versionAvailable = hasObservedInventoryVersion(item); const versionNoticeId = `inventory-version-${item.id}`;
    const identity = inventoryCandidateLabel(item, items); const identityText = inventoryCandidateText(item, items);
    const assessment = webInventoryAssessment(item);
    return <tr key={item.id} className={inspectedId === item.id ? "is-inspected" : undefined} tabIndex={onInspectItem ? (inspectedId === item.id || !inspectedId && index === 0 ? 0 : -1) : undefined} onFocus={(event) => { if (event.target === event.currentTarget) onInspectItem?.(item.id); }} onClick={(event) => { if (!(event.target as HTMLElement).closest("button, input, label")) onInspectItem?.(item.id); }} onDoubleClick={(event) => { if (!(event.target as HTMLElement).closest("input, label, .row-open")) onSelectItem(item.id); }} onKeyDown={(event) => moveInspector(event, index)}>
      <td className="select-column"><label className="inventory-checkbox-hit"><input type="checkbox" className="inventory-checkbox" checked={selectedIds.has(item.id)} onChange={() => onToggleSelected(item.id)} disabled={!versionAvailable} aria-describedby={!versionAvailable ? versionNoticeId : undefined} aria-label={`Select ${identityText}`} /></label>{!versionAvailable && <span id={versionNoticeId} className="sr-only">Cannot select for bulk edit because this row has no positive observed version. Reload inventory first.</span>}</td>
      <td><button className="table-item" aria-pressed={onInspectItem ? inspectedId === item.id : undefined} onClick={() => onInspectItem ? onInspectItem(item.id) : onSelectItem(item.id)}><span className={`item-glyph accent-${item.accent}`}><Icon name={categoryIcons[item.category]} size={16} /></span><span><strong>{identity.name}</strong>{identity.discriminator ? <small>{identity.discriminator}</small> : item.variant ? <small>{item.variant}</small> : null}{(item.category === "Filament" || item.category === "Printers") && <small className={`exact-product-state ${isExactProductConfirmed(item) ? "is-confirmed" : ""}`}>{exactProductLabel(item)}</small>}</span></button></td>
      {visibleColumns.includes("category") && <td className="inventory-category-column"><span className="category-label">{categoryLabel}</span></td>}
      <td className="quantity-cell"><strong>{formatQuantity(item.quantity, item.unit)}</strong>{!assessment.confirmed && <small>Unconfirmed</small>}</td>
      <td className="quantity-cell">{item.unitStatus === "needs_correction" ? <><strong className="quantity-blocked">Fix unit</strong><small>Quantity not usable</small></> : <><strong>{item.availableQuantity === undefined ? "—" : formatQuantity(item.availableQuantity, item.unit)}</strong>{item.reserved > 0 && <small>{formatQuantity(item.reserved, item.unit)} reserved</small>}</>}</td>
      {visibleColumns.includes("reserved") && <td className="quantity-cell">{formatQuantity(item.reserved, item.unit)}</td>}
      <td><span className={`stock-indicator ${assessment.check ? "is-check" : assessment.available ? "is-available" : ""}`}>{inventoryStockLabel(item)}</span></td>
      {visibleColumns.includes("location") && <td className="inventory-location-column"><span className="location-label">{inventoryLocationLabel(item.location)}</span></td>}{visibleColumns.includes("sku") && <td className="inventory-sku-column">{item.sku || "—"}</td>}
      <td className="inventory-row-actions"><div><button className="row-open" onClick={() => onSelectItem(item.id)} aria-label={`Open ${identityText}`}><Icon name="chevron-right" size={17} /></button></div></td>
    </tr>;
  })}</tbody></table></div>;
}
...
function ProjectPage({ pcbSupported, assemblySupported, onImportBom, homeTask, project, projects, projectView, archivedProjectCount, items, offers, tab, expert, sampleMode, reconciliationSupported, makerWorkflowsSupported, onTabChange, onSelectProject, onProjectViewChange, onOpenItem, onNavigate, onToast, onNewProject, onArchive, onRestore, onRemove, onNewRevision, onEditBuildApproach, onRetrySetup, onAddBom, onResolveBomRole, onUpload, onReadReconciliation, onSaveReconciliation, onCommitReconciliation, onRefreshWorkspace, onListInspections, onReadInspection, onPreviewInspection, onConfirmInspection }: {
  ...
  tab: ProjectTab;
  expert: boolean;
  assemblySupported: boolean;
  pcbSupported: boolean;
  makerWorkflowsSupported: boolean;
  ...
  const [localTask, setLocalTask] = useState<(HomeTask & { request: number })>();
  const taskForPlan = localTask?.projectId === project.id && (!homeTask || localTask.request > homeTask.request) ? localTask : homeTask;
  const reviewRequirements = (kind: "check" | "decide") => { setLocalTask({ id: `${project.id}:${kind}`, projectId: project.id, projectName: project.name, kind, label: "Review requirements", detail: "", count: 1, request: Date.now() }); onTabChange("plan"); };
  ...
  const inspectorVisible = detailsOpen && tab !== "assembly" && tab !== "pcb";
  ...
  return ( <InspectionContext.Provider value={inspectionContext}><>
    <PageHeader eyebrow="Project" title={project.name} description={project.description} collapseDescription action={project.status === "archived" ? undefined : "New revision"} onAction={project.status === "archived" ? undefined : onNewRevision}>
      <button type="button" className="button button-quiet" onClick={onNewProject}>New project</button>
      {tab !== "assembly" && tab !== "pcb" && <button type="button" className="button button-secondary project-details-toggle" aria-expanded={detailsOpen} aria-controls="project-inspector" onClick={() => setDetailsOpen((open) => !open)}><Icon name="panel-left" size={16} />Project details</button>}
    </PageHeader>
    ...
    <ProjectManagementBar /><div className="project-task-strip" aria-label="Project task shortcuts">{summary.readinessUnavailable ? <button type="button" onClick={() => { void onRefreshWorkspace(); }}>Refresh stock results</button> : <><span><strong>{summary.readyLines}</strong> Ready</span><button type="button" disabled={!summary.decideLines} onClick={() => reviewRequirements("decide")}><strong>{summary.decideLines}</strong> Decide</button><button type="button" disabled={!summary.inspectLines} onClick={() => reviewRequirements("check")}><strong>{summary.inspectLines}</strong> Check</button><button type="button" disabled={!summary.sourceLines} onClick={() => onTabChange("offers")}><strong>{summary.sourceLines}</strong> Source</button></>}{project.status !== "archived" && routeNeedsDecision && <button type="button" onClick={onEditBuildApproach}>Review build approach</button>}</div> {expert && ( <BuildRail currentStep={project.railStep} projectName={`${project.name} · ${project.currentRevision}`} /> )} <div className={`dossier-layout ${inspectorVisible ? "has-inspector" : "without-inspector"}`}><section className="dossier-workspace"><div className="tab-list" role="tablist" aria-label="Project workspace">{availableTabs.map((target) => <button key={target} ref={(node) => { tabRefs.current[target] = node; }} id={`project-tab-${target}`} role="tab" aria-controls="project-tabpanel" aria-selected={tab === target} tabIndex={tab === target ? 0 : -1} className={tab === target ? "is-active" : ""} onKeyDown={(event) => moveTabFocus(event, target)} onClick={() => onTabChange(target)}>
  <Icon name={target === "plan" ? "clipboard" : target === "files" ? "folder" : target === "offers" ? "tag" : target === "build" ? "layers" : "check-circle"} size={16} />
  {target === "plan" ? "Plan" : target === "files" ? "Files" : target === "offers" ? "Shopping list" : target === "build" ? "Build planning" : target === "assembly" ? "Assembly" : target === "pcb" ? "PCB" : "Update used stock"}
  {target !== "build" && target !== "assembly" && target !== "pcb" && <span>{target === "plan" ? summary.totalLines : target === "files" ? project.artifacts.length : target === "offers" ? summary.sourceLines : reconciliation?.status === "committed" ? "Done" : "Review"}</span>}
</button>)}</div>

 <div id="project-tabpanel" role="tabpanel" aria-labelledby={"project-tab-" + tab} tabIndex={0} > {tab === "pcb" && (pcbSupported ? <PcbWorkspace key={`${project.id}:${project.serverRevisionId}`} project={project} onFiles={() => onTabChange("files")} onAssembly={() => onTabChange("assembly")} /> : <p>PCB viewing is unavailable on this host.</p>)}{tab === "assembly" && (assemblySupported ? <AssemblyWorkspace key={`${project.id}:${project.serverRevisionId}`} project={project} onFiles={() => onTabChange("files")} /> : <p>Assembly viewing is unavailable on this host.</p>)}{tab === "plan" && ( <ProjectPlan homeTask={taskForPlan} onImport={makerWorkflowsSupported && project.status !== "archived" ? onImportBom : undefined} project={project} summary={summary} expert={expert} onOpenItem={onOpenItem} onAddBom={project.status === "archived" ? () => onToast("Restore this project before adding a requirement.") : onAddBom} onResolveBomRole={onResolveBomRole} /> )}{tab === "build" && !makerWorkflowsSupported && <div className="surface workflow-unavailable"><h2>Build planning is unavailable</h2><p>This workspace does not provide build planning.</p><button type="button" className="button button-secondary" onClick={() => onTabChange("plan")}>Back to plan</button></div>}{tab === "build" && makerWorkflowsSupported && <MakerPlanningTools key={`${project.id}:${project.serverRevisionId}`} project={project} items={items} onRefresh={onRefreshWorkspace} />}{" "}{tab === "files" && ( <ProjectFiles project={project} expert={expert} sampleMode={sampleMode} onUpload={(file, role, target) => onUpload(project.id, file, role, target)} archived={project.status === "archived"} /> )}{" "}{tab === "offers" && (makerWorkflowsSupported ? <><ProjectQuoteTools key={`${project.id}:${project.serverRevisionId}`} project={project} onPlan={() => onTabChange("plan")} /><details className="surface historical-offers"><summary>Inventory-linked supplier records</summary><p>These offers belong to inventory records. They are separate from quotes selected for this project.</p><ShoppingList project={project} summary={summary} offers={offers} expert={expert} onToast={onToast} onBackToPlan={() => onTabChange("plan")} /></details></> : <ShoppingList project={project} summary={summary} offers={offers} expert={expert} onToast={onToast} onBackToPlan={() => onTabChange("plan")} />)}{" "}{tab === "reconciliation" && hasServerRevision && ( <section className="reconciliation-page-surface">{reconciliationLoading && ( <div className="reconciliation-loading" role="status"><span className="eyebrow">Update used stock</span><strong>Loading the current review…</strong><p>Nothing changes in stock while this review loads.</p></div> )}{" "}{reconciliationError && !reconciliationLoading && ( <div className="reconciliation-loading reconciliation-load-error" role="alert"><span className="eyebrow">Could not load stock update</span><strong>{reconciliationLoadErrorMessage(reconciliationError, expert)}</strong><button className="button button-secondary" onClick={() => onTabChange("plan")}> {" "}Back to plan{" "} </button></div> )}{" "}{reconciliation && !reconciliationLoading && !reconciliationError && ( <ReconciliationUI embedded readOnly={project.status === "archived"} model={reconciliation} expert={expert} onChange={setReconciliation} onRequestPreview={saveReconciliation} onConfirmCommit={commitReconciliation} /> )}</section> )}</div> </section><aside id="project-inspector" className="dossier-column" hidden={!inspectorVisible} aria-label="Project details"> {expert && ( <div className="dossier-status"><span className={`status-pill tone-${project.status === "complete" ? "good" : "info"}`}><span className="status-symbol">●</span>{projectLifecycleLabel(project.status)}</span><span className="revision-label">{project.currentRevision}</span></div> )} <h2>{project.workItem}</h2><div className="dossier-next" role={routeNeedsDecision ? "region" : undefined} aria-label={routeNeedsDecision ? "Build approach" : undefined}><span className="eyebrow">Next action</span><strong>{nextActionTitle}</strong><span>{nextActionDescription}</span>{project.status !== "archived" && !summary.readinessUnavailable && (routeNeedsDecision ? <button type="button" className="button button-secondary" onClick={onEditBuildApproach}>Set build approach</button> : summary.totalLines === 0 ? <button type="button" className="button button-secondary" onClick={onAddBom}>Add first requirement</button> : summary.decideLines ? <button type="button" className="button button-secondary" onClick={() => reviewRequirements("decide")}>Review missing details</button> : summary.inspectLines ? <button type="button" className="button button-secondary" onClick={() => reviewRequirements("check")}>Review stock checks</button> : summary.sourceLines > 0 ? (tab !== "offers" ? <button type="button" className="button button-secondary" onClick={() => onTabChange("offers")}>Open shopping list</button> : null) : printedRouteNeedsPrinter ? <button type="button" className="button button-secondary" onClick={onEditBuildApproach}>Review printer setup</button> : null)} </div> {!routeNeedsDecision && ( <BuildApproachCard project={project} items={items} expert={expert} onSelectPrinter={onOpenItem} onChoosePrinter={onEditBuildApproach} onReviewPrintItems={(target) => onTabChange(target)} /> )} {expert && ( <><dl className="dossier-facts"><div><dt>Current revision</dt><dd>{project.currentRevision}</dd></div><div><dt>Build files</dt><dd>{project.artifacts.length} artifacts</dd></div><div><dt>Last changed</dt><dd>{project.updated}</dd></div></dl>{project.buildConfigSnapshot && ( <BuildSetupSummary input={project.buildConfigSnapshot} printer={configuredPrinter} filament={configuredFilament} expert /> )} </> )}{" "}{expert && ( <details className="expert-detail"><summary>Technical context</summary><ProjectExpertContext project={project} /></details> )}<button className="text-button dossier-inventory-link" onClick={() => onNavigate("inventory")}> {" "}Browse all inventory <Icon name="arrow-right" size={15} />{" "} </button><details className="project-actions"><summary> {" "}Project settings <Icon name="chevron-down" size={14} /></summary><div className="project-settings-content"><span>{expert ? "Archive keeps revisions and audit history available. Removing a project keeps its tombstone but cannot be undone." : "Archive keeps the project and its history available. Removing a project cannot be undone, although its project history is kept."}</span><div className="project-settings-buttons"><button type="button" className="button button-danger" onClick={() => setRemoveConfirmationOpen(true)}><Icon name="trash" size={16} />Delete project</button>{project.status === "archived" ? ( <button className="button button-primary" onClick={() => setRestoreConfirmationOpen(true)}><Icon name="refresh" size={16} /> Restore project{" "} </button> ) : ( <button className="button button-secondary" onClick={() => setArchiveConfirmationOpen(true)}><Icon name="archive" size={16} /> Archive project{" "} </button> )}</div></div></details></aside></div>
  </></InspectionContext.Provider> );
}

function ProjectPlan({ homeTask, onImport, project, summary, expert, onOpenItem, onAddBom, onResolveBomRole }: { homeTask?: (HomeTask & { request: number }) | undefined; onImport?: (() => void) | undefined; project: Project; summary: ReturnType<typeof calculateProjectSummary>; expert: boolean; onOpenItem: (id: string) => void; onAddBom: () => void; onResolveBomRole: ( lineId: string, role: "consumed" | "reusable", expectedVersion: number ) => Promise<void>; }) {
  ...
  useEffect(() => { setRequirementQuery(""); setRequirementFilter("all"); }, [project.id, project.serverRevisionId]);
  const planRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    if (!homeTask || homeTask.projectId !== project.id) return;
    setRequirementQuery(""); setRequirementFilter(homeTask.kind === "check" ? "check" : homeTask.kind === "decide" ? "decide" : "all");
    const timer = window.setTimeout(() => { const target = planRef.current?.querySelector<HTMLElement>(".bom-section"); target?.focus({ preventScroll: true }); target?.scrollIntoView({ block: "start", behavior: "instant" }); }, 0);
    return () => window.clearTimeout(timer);
  }, [homeTask?.request, project.id]);
  const visibleLines = summary.lineStatuses.filter((line) => matchesRequirementFilter(line, requirementQuery, requirementFilter));
  const filtering = requirementQuery.trim() !== "" || requirementFilter !== "all";

  return ( <div className="project-plan" ref={planRef}> {(expert || (inspection?.actions.length ?? 0) > 0 || inspection?.error) && ( <InspectionQueuePanel actions={inspection?.actions ?? (project as InspectionProject).inspectionActions ?? []} expert={expert} loadError={inspection?.error} onReadInspection={inspection?.onReadInspection} onPreviewInspection={inspection?.onPreviewInspection} onConfirmInspection={inspection?.onConfirmInspection} /> )} <section className="surface bom-section" tabIndex={-1}><div className="requirements-heading"><SectionHeading eyebrow="Requirements" title="Required parts and materials" /><div className="requirements-actions">{onImport && <button type="button" className="button button-quiet" onClick={onImport}><Icon name="upload" size={15} />Import requirements from CSV</button>}<button className="add-line-button" onClick={onAddBom}><Icon name="plus" size={16} />{empty ? "Add first requirement" : "Add a requirement"}</button></div></div>{(summary.totalLines > 6 || filtering) && <div className="requirement-filters"><label className="form-field"><span>Search requirements</span><input aria-label="Search project requirements" value={requirementQuery} onChange={(event) => setRequirementQuery(event.target.value)} placeholder="Part, note or selected item" /></label><label className="form-field"><span>Show</span><select aria-label="Filter project requirements" value={requirementFilter} onChange={(event) => setRequirementFilter(event.target.value as RequirementFilter)}>{[["all", "All requirements"], ["attention", "Needs attention"], ["ready", "Ready"], ["check", "Needs checking"], ["decide", "Needs a decision"], ["source", "Needs sourcing"], ["optional", "Optional"]].map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><p role="status">Showing {visibleLines.length} of {summary.totalLines} requirements. Readiness and exports still use the full plan.</p>{filtering && <button type="button" className="text-button" onClick={() => { setRequirementQuery(""); setRequirementFilter("all"); }}>Clear requirement filters</button>}</div>} {expert && ( <div className="bom-explainer"><Icon name="info" size={16} /><span> {" "}Counted or commissioned stock is Ready. Delivered or ordered stock is Check. Missing details are Decide. Stock that is still missing is Source.{" "} </span></div> )}{" "} {empty ? ( <div className="empty-state"><h3>No requirements are recorded yet.</h3><p>Add the parts, materials, and tools this build needs. Use the Files tab for designs and instructions.</p></div> ) : ( <div className="bom-list"><div className="bom-grid-head" aria-hidden="true"><span>Requirement</span><span>Quantity</span><span>Stock / status</span><span /></div>{visibleLines.length === 0 && <p className="requirements-no-match">No requirements match these filters.</p>}{visibleLines.map((line) => ( <BomLineRow key={line.line.id} line={line} expert={expert} onOpenItem={onOpenItem} onResolveRole={onResolveBomRole} />))}</div> )}</section><RemovedRequirements /> {expert && ( <section className="surface learning-section"><SectionHeading eyebrow="Project memory" title="Recorded observations" /><div className="learning-list">{project.notes.length ? ( project.notes.map((note, index) => ( <div className="learning-row" key={note}><span className="learning-index">0{index + 1}</span><p>{note}</p><span className="learning-time">Recorded</span></div>)) ) : ( <p className="activity-empty"> {" "}No observations are recorded for this revision yet.{" "} </p> )}</div></section> )} </div> );
}
...
export function ProjectFiles({ project, expert, sampleMode, onUpload, archived = false }: { project: Project; expert: boolean; sampleMode: boolean; onUpload: (file: File, role: string, target?: ArtifactUploadTarget) => Promise<void>; archived?: boolean; }) {
  ...
  const scopeArtifacts = filterArtifactsForScope(allArtifacts, scope);
  const visibleArtifacts = scopeArtifacts.filter((file) => `${file.name} ${file.role}`.toLocaleLowerCase().includes(fileQuery.trim().toLocaleLowerCase()));
  const uploadsDisabled = archived || uploadRun?.active === true || scope.kind === "all";
  const fileNavigation = useContext(UnsavedWorkContext);
  useUnsavedWork(selectedFiles.length > 0, "selected files", uploadRun?.active === true);
  ...
  const scopeDescription = expert
    ...
    <div className="artifact-scope-control"><label className="form-field" htmlFor="artifact-scope"><span>File scope</span><select id="artifact-scope" aria-label="Choose file scope" value={selectedChoice?.key ?? "all"} onChange={(event) => { const next = event.target.value; const change = () => { setScopeKey(next); setSelectedFiles([]); }; if (selectedFiles.length && fileNavigation) fileNavigation.request(change); else change(); }} disabled={uploadRun?.active || archived}>{choices.map((choice) => ( <option key={choice.key} value={choice.key} disabled={choice.disabled}>{choice.label}</option>))}</select></label><div className="file-scope-identity"><Icon name="folder" size={15} /><span><strong>{artifactScopeIdentity(scope, expert)}</strong><code>{scopeDescription}</code></span><span className="file-scope-context">{sampleMode ? "sample workspace" : "private workspace"}</span></div></div>
    {!archived && <div className={`files-drop-area ${dropActive ? "is-dragging" : ""} ${uploadsDisabled ? "is-disabled" : ""}`} role="group" aria-label="File drop area"
      onDragOver={(event) => { if (Array.from(event.dataTransfer.types).includes("Files")) { event.preventDefault(); if (!uploadsDisabled) setDropActive(true); } }}
      ...
      onDrop={(event) => { event.preventDefault(); setDropActive(false); if (!uploadsDisabled) { const selected = Array.from(event.dataTransfer.files); if (selected.length) setSelectedFiles(selected); } }}>
      <Icon name="upload" size={22} /><div><strong>{scope.kind === "all" ? "Select a revision to add files" : selectedFiles.length ? `${selectedFiles.length} files selected` : "Drop design files here"}</strong><span>{scope.kind === "all" ? "This view is read-only." : "Review the selection, then select Add files. Nothing uploads on drop."}</span></div>
      {selectedFiles.length > 0 && <button type="button" className="text-button" disabled={uploadsDisabled} onClick={() => setSelectedFiles([])}>Clear file selection</button>}
    </div>}
...
export function ShoppingList({ project: _project, summary, offers, expert, onToast, onBackToPlan }: { project: Project; summary: ReturnType<typeof calculateProjectSummary>; offers: typeof fixtureOffers; expert: boolean; onToast: (message: string, tone?: ToastTone) => void; onBackToPlan: () => void; }) {
  ...
  return ( <>
    ...
    <section className="agent-callout"><div className="agent-callout-icon"><Icon name="code" size={21} /></div><div><strong>Read capabilities before using tools.</strong><p> {" "}Use inventory and project evidence to identify reuse, required checks, and missing parts.{" "} </p></div><span className="api-status">Live contract</span></section>
    ...
    return () => {
      ...
      if (previousFocus?.isConnected) {
        ...
        const restoreFocus = () => {
          ...
          // Respect focus set by a later navigation or newly opened dialog.
          if (focused instanceof HTMLElement && focused !== document.body && focused !== previousFocus && focused.isConnected && !container?.contains(focused) && !focused.closest("[inert]")) return;
...
export function InventoryDrawer({ item, suspended = false, sampleMode = false, items = [item], categories, categoriesLoading, categoriesError, expert, onClose, onCount, onCommission, onUpdate, onDelete, onLinkProduct, onCreateReplacement }: { item: InventoryItem; sampleMode?: boolean; suspended?: boolean; items?: readonly InventoryItem[]; categories: readonly ManagedInventoryCategory[]; categoriesLoading: boolean; categoriesError?: string | undefined; expert: boolean; onClose: () => void; onCount: (id: string, quantity: number) => Promise<InventoryItem>; onCommission: (id: string, input: InventoryCommissionInput, expectedVersion: number) => Promise<InventoryItem>; onUpdate: (id: string, input: Partial<InventoryUpdateInput>, expectedVersion?: number) => Promise<InventoryItem>; onDelete?: (item: InventoryItem) => Promise<void>; onLinkProduct?: (item: InventoryItem) => void; onCreateReplacement?: (item: InventoryItem) => void; }) {
  ...
  const confirmDelete = async () => {
    ...
    try { await onDelete(item); setDeleteOpen(false); }
    catch (error: unknown) { setDeleteError(normalizeApiError(error).message); }
  ...
  return ( <>
    ...
    {deleteOpen && <Dialog title={`Delete ${item.name}?`} role="alertdialog" onClose={() => { if (!deleting) setDeleteOpen(false); }}>
      <p className="dialog-intro">This removes the item from inventory and printer choices. Its stock records and project history are kept. Existing plans may need a replacement. This cannot be undone.</p>
      {deleteError && <p className="form-error" role="alert">{deleteError}</p>}
      <div className="dialog-actions"><button type="button" className="button button-quiet" disabled={deleting} onClick={() => setDeleteOpen(false)}>Cancel</button><button type="button" className="button button-danger" disabled={deleting || item.version === undefined} aria-busy={deleting} onClick={() => { void confirmDelete(); }}>{deleting ? "Deleting…" : item.kind === "printer" ? "Delete printer" : "Delete item"}</button></div>
      {item.version === undefined && <p role="alert">Reload this item before deleting it.</p>}
    </Dialog>}
...
function InventoryMutationReviewDialog({ item, review, saving, onClose, onConfirm }: { item: InventoryItem; review: InventoryMutationReview; saving: boolean; onClose: () => void; onConfirm: () => void; }) {
...
} export function NewProjectDialog({ items = [], suspended = false, onClose, onAddPrinter, onCreate, onGuided }: { items?: InventoryItem[]; suspended?: boolean; onGuided?: (() => void) | undefined; onClose: () => void; onAddPrinter?: (() => void) | undefined; onCreate: (input: ProjectCreateInput) => Promise<ProjectCreateOutcome>; }) {
  ...
  const printers = items.filter(isUsableOwnedPrinter); const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (typeof outcome === "object") setFormError(outcome.error);
      ...
      setSubmitting(false);
  ...
  }; return ( <Dialog title="Create project" onClose={onClose}>{onGuided && <button type="button" className="button button-secondary setup-entry" onClick={onGuided}>Use a template or import a BOM</button>}<form onSubmit={(event) => { void submit(event); }}><p className="dialog-intro"> {" "}
          ...
          can change this later.{" "} </p> <label className="form-field"> <span>Project name</span> <input autoFocus required value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. Desk sensor enclosure" disabled={submitting} /> </label><label className="form-field"><span>Project goal</span><textarea required value={description} onChange={(event) => setDescription(event.target.value)} rows={4} placeholder="Describe the required result" disabled={submitting} /></label><fieldset className="fabrication-route-options" aria-describedby="fabrication-route-help" ><legend>How will you build it?</legend> {fabricationRouteOptions.map((option) => ( <label className={`fabrication-route-option ${fabricationRoute === option.value ? "is-selected" : ""}`} key={option.value}><input type="radio" name="fabrication-route" value={option.value} checked={fabricationRoute === option.value} onChange={() => setFabricationRoute(option.value)} disabled={submitting} /><span> <strong>{option.label}</strong><small>{option.description}</small> </span></label> ))} <p id="fabrication-route-help"> {" "}
            This selection does not buy, reserve, or build
            anything.{" "} </p> </fieldset> {fabricationRoute === "printed" && ( <div className="route-printer-picker"> {printers.length ? ( <OwnedItemCombobox category="Printers" items={printers} value={printer} onSelect={setPrinter} label="Printer for this project" helper="Leave blank if you have not decided yet." showInitialChoices /> ) : ( <div className="printer-reassurance"> <strong>No owned printer recorded yet.</strong><span> {" "}
                  Add a printer later, or use ready-made parts or electronics-only assembly.{" "} </span> {onAddPrinter && ( <button type="button" className="text-button" onClick={onAddPrinter} disabled={submitting} > {" "}
...
const exactBuildConfig = expert && printer && isUsableOwnedPrinter(printer) && printerEligibility.eligible && !printerNeedsDetails && filamentEligibility.eligible ? buildConfig : undefined; const created = await onCreate( revisionInputForRoute({ name, status: expert ? status : "concept", notes, fabricationRoute, ...(printer?.id === undefined ? {} : { intendedPrinterItemId: printer.id }), printerSelectionTouched: printerChanged, ...(exactBuildConfig ? { buildConfig: exactBuildConfig } : {}) }) ); if (!created) setFormError( "The revision was not created. Check the service connection and try again." ); } catch (error: unknown) { setFormError(normalizeApiError(error).message); } finally { setSubmitting(false); } }; return ( <Dialog title={`New revision for ${project.name}`} onClose={onClose}> <form onSubmit={(event) => { void submit(event); }} > <p className="dialog-intro"> {" "}
          ...
          approach forward and change it later.{" "} </p> <label className="form-field"> <span>Revision name</span> <input autoFocus required value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. R02 enclosure fit" disabled={submitting} /> </label> <fieldset className="fabrication-route-options" aria-describedby="revision-fabrication-route-help" > <legend>How will you build it?</legend> {fabricationRouteOptions.map((option) => ( <label className={`fabrication-route-option ${fabricationRoute === option.value ? "is-selected" : ""}`} key={option.value} > <input type="radio" name="revision-fabrication-route" value={option.value} checked={fabricationRoute === option.value} onChange={() => setFabricationRoute(option.value)} disabled={submitting} /> <span> <strong>{option.label}</strong> <small>{option.description}</small> </span> </label> ))} <p id="revision-fabrication-route-help"> {" "}
            ...
            build anything.{" "} </p> </fieldset> {fabricationRoute === "printed" && ( <> <div className="setup-picker-grid"> <OwnedItemCombobox category="Printers" items={selectablePrinters} value={printer} onSelect={(selection) => { setPrinter(selection); setPrinterChanged(true); }} onResolveItem={ printerNeedsDetails ? onAddPrinterDetails : undefined } label="Printer for this revision" helper="Leave blank if you have not decided yet." /> {expert && ( <OwnedItemCombobox category="Filament" items={items} value={filament} onSelect={setFilament} label="Filament (optional technical setup)" /> )} </div> {!printer && ( <div className="printer-reassurance"> <strong>No printer selected yet.</strong> <span> {" "}
                  Choose or add an owned printer when needed.{" "} </span> {onAddPrinter && ( <button type="button" className="text-button" onClick={onAddPrinter} disabled={submitting} > {" "}
...
const bomUnitOptions: readonly { value: BomInput["unit"]; label: string }[] = [
...
]; export function AddBomDialog({ items, project, expert, onClose, onCreate }: { items: InventoryItem[]; project: Project; expert: boolean; onClose: () => void; onCreate: (input: BomInput) => Promise<boolean | { error: string }>; }) {
  const [name, setName] = useState("");
  ...
  const submit = async (event: FormEvent) => {
    ...
    setFormError(undefined);
    try {
      const created = await onCreate({ name: name.trim(), requiredQuantity, unit, role, ...(itemId ? { itemId } : {}), ...(optional ? { optional: true } : {}), ...(note.trim() ? { note: note.trim() } : {}) });
      if (typeof created === "object") setFormError(created.error);
      else if (!created) setFormError("The requirement was not added. Check the service connection and try again.");
    } catch (error: unknown) {
      setFormError(normalizeApiError(error).message);
    } finally {
      setSubmitting(false);
  ...
  const filteredItems = items.filter((item) => {
    ...
    return item.category !== "Printers" && (item.id === itemId || !needle || [item.name, item.variant, item.location, item.manufacturer, item.sku, inventoryDiscriminator(item)].filter(Boolean).join(" ").toLocaleLowerCase().includes(needle) );
  });
  const selectedItem = items.find((item) => item.id === itemId);
  return ( <Dialog title={ expert ? `Add a requirement to ${project.currentRevision}` : "Add a part, material, or tool" } onClose={onClose}><form onSubmit={(event) => { void submit(event); }}><p className="dialog-intro"> {expert ? "Describe one requirement and optionally choose stock you already own. Its availability and compatibility will be checked after saving." : "Tell BenchLedger what you need. If you already own it, choose the item below so it can be checked before you shop."} </p><label className="form-field"><span>What do you need?</span><input autoFocus required value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. ESP32 development board" disabled={submitting} /></label><div className="form-row"><label className="form-field"><span>Quantity</span><input type="number" min="0.01" step="any" required value={quantity} onChange={(event) => setQuantity(event.target.value)} disabled={submitting} /></label><label className="form-field"><span>Unit</span><select value={unit} onChange={(event) => setUnit(event.target.value as BomInput["unit"])} disabled={submitting}>{bomUnitOptions.map((option) => ( <option key={option.value} value={option.value}>{option.label}</option>))}</select></label></div><label className="form-field"> <span>How will you use it?</span> <select aria-label="How will you use it?" required value={role} onChange={(event) => setRole(event.target.value as NonNullable<BomInput["role"]>) } disabled={submitting} > <option value="consumed"> {" "}
              Part or material (used up or built in){" "} </option> <option value="reusable">Reusable tool or equipment</option> </select> </label> {( <div className="form-field matching-stock-field"><span> {" "}Use an item I already own <small>(optional)</small></span><input aria-label="Search matching inventory" value={itemSearch} onChange={(event) => setItemSearch(event.target.value)} placeholder="Search name, colour, location, or SKU" disabled={submitting} /><select aria-label="Choose matching inventory" value={itemId} onChange={(event) => setItemId(event.target.value)} disabled={submitting}><option value="">No owned item selected</option>{filteredItems.map((item) => ( <option key={item.id} value={item.id}>{inventoryCandidateText(item, items)}</option>))}</select>{selectedItem && ( <small className="matching-stock-hint"> {" "}Selected: {inventoryCandidateText(selectedItem, items)}. Choose this item only if it meets the requirement. Stock is recorded in {inventoryUnitLabels[selectedItem.unit]}.</small> )} </div> )}{" "} {expert && ( <details className="expert-detail requirement-expert-detail" open> <summary>Technical details</summary> <div className="detail-grid"> <div> <span>Project revision</span> <code>{project.serverRevisionId ?? "Not recorded"}</code> </div> <div> <span>Revision label</span> <code>{project.currentRevision}</code> </div><div> <span>Requirement role</span> <code>{role}</code> </div> <div> <span>Matching</span> <code> {itemId ? "Manual item selected" : "No inventory item selected"}</code> </div></div> </details> )} <label className="form-field"><span> {" "}Requirement note <small>(optional)</small></span><textarea value={note} onChange={(event) => setNote(event.target.value)} rows={2} placeholder="Fit, material, or compatibility detail" disabled={submitting} /></label><label className="check-field"><input type="checkbox" checked={optional} onChange={(event) => setOptional(event.target.checked)} disabled={submitting} /><span>Mark as optional</span></label>{formError && ( <p className="form-error" role="alert">{formError}</p> )}<div className="dialog-actions"><button type="button" className="button button-quiet" onClick={onClose} disabled={submitting}> {" "}Cancel{" "} </button><button type="submit" className="button button-primary" disabled={!name.trim() || submitting} aria-busy={submitting}>{submitting ? "Adding…" : "Add requirement"} {" "} {!submitting && <Icon name="arrow-right" size={16} />}</button></div></form></Dialog> );
}
