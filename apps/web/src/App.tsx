...
export function formatSourceReadyMessage(count: number): string {
  ...
  const overlayOpen = Boolean(commandsOpen || showGuidedSetup || editingProject || editingRequirement || selectedItem || showNewProject || showNewRevision || showEditBuildApproach || showAddBom || showNewItem || bulkInventorySelection);
  useEffect(() => {
  ...
  const commandItems: WorkspaceCommand[] = [
    ...
    ...projects.map((project) => ({ id: `project:${project.id}`, label: project.name, detail: project.currentRevision, group: "Projects" as const, icon: "folder" as const, run: () => openProject(project.id) })),
  ...
  return (
    <div className="app-shell">
      <a className="skip-link" href="#main-content" onClick={(event) => { event.preventDefault(); mainRef.current?.focus(); }}>Skip to workspace</a>
...
function projectFabricationRoute(project: Project): FabricationRoute { const plannedPrinter = (project as NullablePrinterProject).intendedPrinterItemId; if (project.fabricationRoute) return project.fabricationRoute; if (plannedPrinter !== undefined) return plannedPrinter ? "printed" : "undecided"; return project.buildConfigSnapshot?.printerItemId ? "printed" : "undecided"; } function projectIntendedPrinterId(project: Project): string | undefined { const plannedPrinter = (project as NullablePrinterProject).intendedPrinterItemId; if (plannedPrinter !== undefined) return plannedPrinter ?? undefined; return project.fabricationRoute === undefined ? project.buildConfigSnapshot?.printerItemId : undefined; } export function isUsableOwnedPrinter(item: InventoryItem): boolean { const evidence = item.serverEvidence ?? item.evidence; const retired = (item as InventoryItem & { retired?: boolean }).retired === true || item.tags.some((tag) => tag.toLocaleLowerCase() === "retired");
  ...
  const nextActionDescription = activeProject
    ? readinessUnavailable
      ? "The latest inventory results are unavailable. Reload before preparing a Source proposal." : activeRouteNeedsDecision ? "Select 3D printing, ready-made parts or electronics assembly." : noRequirements
  ...
  return <div className="workbench-overview">
    <PageHeader eyebrow="Workspace" title="Workspace overview" description="Project status, stock checks and workshop equipment.">
      <button className="button button-primary" type="button" onClick={onNewProject}><Icon name="plus" size={16} />New project</button>
    </PageHeader>
    <WorkbenchMetrics projects={projects} items={items} />
    <div className="workbench-grid">
      <div className="workbench-main">
        <WorkbenchRegister projects={projects} items={items} onOpen={(id) => onOpenProject(id)} />
        <section className="surface overview-start" aria-labelledby="overview-project-heading">
          <div className="overview-start-copy">
            <span className="eyebrow">{activeProject ? "Next action" : "Project setup"}</span>
            <h2 id="overview-project-heading">{nextActionTitle}</h2>
            <p>{nextActionDescription}</p>
            {activeProject && <div className="overview-next-action"><Icon name="folder" size={15} /><strong>{activeProject.name}</strong><span>{activeProject.currentRevision}</span></div>}
          </div>
          {activeProject && <button className="button button-secondary overview-primary-action" type="button" onClick={() => onOpenProject(activeProject.id)}>Continue {activeProject.name}<Icon name="arrow-right" size={16} /></button>}
        </section>
        {expert && activeProject && <section className="surface overview-expert-context" aria-label="Technical project context">
          <div><span className="eyebrow">Readiness</span><strong>{activeSummary?.readyLines ?? 0} ready · {activeSummary?.checkLines ?? 0} check</strong><small>{activeSummary?.decideLines ?? 0} decide · {activeSummary?.sourceLines ?? 0} source</small></div>
          <div><span className="eyebrow">Revision</span><strong>{activeProject.currentRevision}</strong><small>{activeProject.serverRevisionId ?? "Revision ID not recorded"}</small></div>
          <div><span className="eyebrow">Build setup</span><strong>{configuredPrinter ? inventoryCandidateLabel(configuredPrinter, items, true).name : "No printer setup recorded"}</strong><small>{configuredFilament ? inventoryCandidateLabel(configuredFilament, items, true).name : "No filament setup recorded"}</small></div>
        </section>}
      </div>
      <div className="workbench-side">
        <section className="surface workshop-summary" aria-labelledby="workshop-summary-heading">
          <div className="workshop-summary-header"><div><span className="eyebrow">Equipment</span><h2 id="workshop-summary-heading">Workshop equipment</h2></div><Icon name="tool" size={18} /></div>
          <div className="workshop-summary-grid">
            <div className="workshop-group workshop-printers"><div className="workshop-group-heading"><span><Icon name="layers" size={16} />Owned printers</span><strong>{printers.length}</strong></div>
              {printers.length ? <div className="workshop-printer-list">{printers.map((item) => <button type="button" className="workshop-printer-card" key={item.id} onClick={() => onSelectItem(item.id)}><span><strong>{inventoryCandidateLabel(item, items, expert).name}</strong><small>{printerBuildVolumeCopy(item) ?? "Build volume not recorded"}</small></span><Icon name="arrow-up-right" size={14} /></button>)}</div> : <div className="workshop-empty-state"><span className="workshop-empty">No owned printers recorded. Electronics and ready-made builds do not need a printer.</span>{onAddPrinter && <button type="button" className="text-button" onClick={onAddPrinter}>Add printer<Icon name="plus" size={14} /></button>}</div>}
            </div>
            <div className="workshop-group workshop-tools"><div className="workshop-group-heading"><span><Icon name="tool" size={16} />Tools</span><strong>{tools.length}</strong></div>{toolNames.length ? <div className="workshop-name-list">{toolNames.map((name) => <span key={name}>{name}</span>)}{tools.length > toolNames.length && <small>{tools.length - toolNames.length} more tools</small>}</div> : <span className="workshop-empty">No tools recorded.</span>}</div>
          </div>
          <div className="workshop-summary-actions"><button type="button" className="text-button" onClick={() => onNavigate("inventory")}>Manage inventory<Icon name="arrow-right" size={15} /></button>{expert && <button type="button" className="text-button workshop-agent-link" onClick={() => onNavigate("capabilities")}>For agents<Icon name="code" size={14} /></button>}</div>
        </section>
        {activeProject && projectFabricationRoute(activeProject) !== "undecided" && <BuildApproachCard project={activeProject} items={items} expert={expert} onSelectPrinter={onSelectItem} onReviewPrintItems={(target) => onOpenProject(activeProject.id, target)} />}
        <section className="surface workbench-reference"><span className="eyebrow">Status key</span><h2>Read the plan</h2><div><span className="status-pill tone-good">Ready</span><p>Confirmed stock covers the requirement.</p></div><div><span className="status-pill tone-warn">Check</span><p>A physical stock check is needed.</p></div><div><span className="status-pill tone-info">Decide</span><p>The requirement needs more detail.</p></div><div><span className="status-pill tone-bad">Source</span><p>The requirement has a stock gap.</p></div></section>
      </div>
    </div>
  </div>;
}
