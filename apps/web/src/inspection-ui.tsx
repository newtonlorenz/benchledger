...
export function InspectionResultDialog({ action, expert, onClose, onPreviewInspection, onConfirmInspection }: {
  ...
  const input = (): InspectionCompletionInput => {
    ...
    const conversion = action.kind === "unit_conversion" && conversionQuantity.trim() && conversionBasis
      ...
      } satisfies NonNullable<InspectionCompletionInput["conversion"]>
      : undefined;
    return {
  ...
  return <div className="inspection-dialog-scrim" role="presentation"><form className="inspection-dialog" role="dialog" aria-modal="true" aria-labelledby={headingId} onSubmit={(event) => { void requestPreview(event); }}>
    ...
    <div className="inspection-dialog-fields"><label className="form-field" htmlFor={`${headingId}-source`}><span>Evidence source</span><input id={`${headingId}-source`} required value={source} onChange={(event) => { setSource(event.target.value); setPreview(undefined); }} placeholder="Physical check, label, or measurement note" disabled={Boolean(pending)} /></label><label className="form-field" htmlFor={`${headingId}-source-id`}><span>Source ID <small>(optional)</small></span><input id={`${headingId}-source-id`} value={sourceId} onChange={(event) => { setSourceId(event.target.value); setPreview(undefined); }} placeholder="Label, record, or document ID" disabled={Boolean(pending)} /></label></div>
